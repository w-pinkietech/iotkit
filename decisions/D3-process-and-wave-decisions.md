...
※明文の例外(D13決定2 2026-07-08): 未コミッショニング箱の**setupモード窓**はoperatorトークンなしで
閉集合の操作を許す(管理者パスフレーズ設定で閉鎖・監査actor=`setup_mode`・工事層と上流接続は開かない)。
...
6. ~~UIスコープ文書~~ **確定(2026-07-08)**: [D13](D13-ui-scope.md)(器と予算・setupモード+
