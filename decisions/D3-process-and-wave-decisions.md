...
0. **BravePI device_number焼き込み/設定可能の実機・仕様確認**(Wave 0のBravePIアダプタ着手前
   ブロッカー、D5宿題。レビュー反映 2026-07-02 で追加)
1. **series識別モデル**(最優先。dedupキー・認可・復旧・出口契約の根元。Wave 0のコード前に確定)
   → **確定候補: D5**
2. ADR 42本(monojoh-authority)の生死棚卸し=権威の一本化。棚卸しで要処理と判明済み(レビュー反映
   2026-07-02): 0003/0006(host-agent境界とアダプタ配置)、0013(series identityの運搬→コレクタ解決へ)、
   0021/0038(mTLS→トークン+TLS)、0033(証明書ライフサイクル書き直し)、0042(集中クレデンシャル
   管理条項)、0025(sqlx指定・コンポーネント分割)、0039(バッチ既定値)
...
4. 測定レジストリの正本二義性の解消(リポジトリ資産か現場ローカルか)+初期語彙。
   **Wave 0ブロッカー**(初期語彙が必要)+構造化値型の要否(spectrogram移行の前提、D5より)
