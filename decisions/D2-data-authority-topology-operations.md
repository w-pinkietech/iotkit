...
- Phase 3 初期設定: スマホ/PCのUIで管理者設定・時刻確認・ネットワーク設定
  (管理者パスフレーズ設定=setupモード終了=D13決定2。setupモード窓ではPhase 3/4の順序が
  入れ替わりうる——体感優先で登録が先でも、Phase 6がパスフレーズ未設定を不合格にして下限を守る)
...
2. **スナップショット内容**(manifest+`format_version` 付き、初版から):
   ①devices/series台帳 ②現場レジストリ ③較正値 ④desired設定 ⑤`secrets` セクション
   (TLS秘密鍵・per-deviceトークンハッシュ・**operatorトークンハッシュ**・**トンネル秘密鍵(ホスト型。
   D10 2026-07-08追加)**。**Wave 1+**)。短命の束縛credentialは含めない——復元後に箱束縛鍵で認証して
   無人再発行で取り直す(D10決定3・決定7)。
   readings本体は対象外(custody transfer/outboxの領分)。
   復元によりトークンは**有効なまま引き継がれる**(復旧直後にAI/人間が制御プレーンへ入れることの保証。
   外部レビュー第2回反映 2026-07-03)。復旧を機に失効させたい場合はR14の明示失効操作を使う——
   エポックフェンスが拒むのは「旧エポックを名乗る箱」であって、新しい箱上の正当なトークンではない。
...
- **アダプタ: デフォルト同梱、ただしアーキテクチャ要件ではない**。
  ...
  - センサーが遠い場合は**衛星アダプタ構成**: RPi Zero等でアダプタランタイムのみ動かし、
    ...
    **経路はサイトLAN内のみ**(D11決定7 2026-07-08: 入口リスナーはインターネット非公開のため、
    インターネット越えの衛星アダプタは構成として認めない。遠隔地は別のIoTゲートウェイ+出口契約で運ぶ)
  - ソフトウェアは同一、デプロイ配置だけの違い
  - **注(D8 2026-07-07)**: この衛星アダプタ(コレクタを持たない薄いランタイム)は、D8のトポロジ2分
    (Standalone/Site-managed)とは**直交する概念**である。D8が却下したのは「rpi4b級の完全ゲートウェイを
    中央1台の衛星にする Model A」であって、RPi Zero級の非コレクタ衛星アダプタではない。複数Piが
    それぞれ完全ゲートウェイになる現場(=Site-managed)の各Piが、さらにその配下にこの衛星アダプタを
    持つことはあり得る。
