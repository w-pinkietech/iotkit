...
**劣化契約との優先順位(2026-07-03確定、外部レビュー第2回反映)**: R17劣化契約のパージ順序は
**①アーカイブ責任消費者ack済み(最低保持フロア超過分)→ ②解決もretireもされない検疫滞留行(D5)→
③未ackの正本**の順。③に到達したときのみ「データ損失」であり、③の実行には
...
2. **スナップショット内容**(manifest+`format_version` 付き、初版から):
   ...
   (TLS秘密鍵・per-deviceトークンハッシュ・**operatorトークンハッシュ**。**Wave 1+**)。
