...
- **target登録の認証・認可(R19の出口面の骨子)**: target登録・購読フィルタ変更・アーカイブ責任
  ...
  アーカイブフラグ付け替えは正本移転先の変更なので最高権限段階)。push接続はTLS+per-target資格
  情報(消費者側がゲートウェイを認証する鍵を含む)。詳細スキームはR19設計スペックの宿題。
  ゲートウェイは登録されたURLへ全測定データをPOSTする以上、登録が認可なしなら1回の誤設定が
...
| target登録・push接続の認証認可スキーム詳細 | R19設計スペック |
