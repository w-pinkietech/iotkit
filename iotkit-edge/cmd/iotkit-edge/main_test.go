...
func TestRunUsageNamesIoTKitEdge(t *testing.T) {
	err := run(nil)
	if err == nil || !strings.Contains(err.Error(), "usage: iotkit-edge ") {
		t.Fatalf("run error = %v, want iotkit-edge usage", err)
	}
	if strings.Contains(err.Error(), "iotkit-edge-server") {
...
func TestRouteListWritesDeliveryStatusJSON(t *testing.T) {
	...
	mapping, err := archive.ApplySemanticMapping(context.Background(), edgeapp.LocalCLIActor(), semantic.MappingSpec{
	...
	route, err := archive.ApplyLegacyMQTTRoute(
		context.Background(), edgeapp.LocalCLIActor(), mapping.ID, "factory/production-pulses",
	)
	if err != nil {
		t.Fatal(err)
	}
...
func TestAccountBootstrapAndRecoveryUseOwnerOnlyPasswordFiles(t *testing.T) {
	...
	account, err := archive.GetAccountByLoginID(context.Background(), "edge.owner")
	if err != nil {
		t.Fatal(err)
	}
	...
	t.Cleanup(func() { _ = archive.Close() })
	recovered, err := archive.GetAccountByLoginID(context.Background(), "edge.owner")
	if err != nil {
		t.Fatal(err)
	}
...
func TestBackupCreateDoesNotCreateMissingSourceDatabase(t *testing.T) {
	...
	if err == nil || !strings.Contains(err.Error(), "existing Edge database") {
