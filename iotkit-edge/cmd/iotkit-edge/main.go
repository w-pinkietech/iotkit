...
import (
	...
	"github.com/w-pinkietech/iotkit-next/iotkit-edge/internal/edgehttp"
	"github.com/w-pinkietech/iotkit-next/iotkit-edge/internal/edgesession"
	"github.com/w-pinkietech/iotkit-next/iotkit-edge/internal/mqttedge"
...
func runBackup(args []string) error {
	if len(args) == 0 {
		return errors.New("usage: iotkit-edge backup <create|restore|accept-archive-loss> [options]")
	}
	switch args[0] {
	case "create":
...
func runBackupCreate(args []string) error {
	...
	defer archive.Close()
	result, err := edgeapp.NewService(archive).Dispatch(
		context.Background(), edgeapp.LocalCLIActor(), edgeapp.CreateEdgeBackup{
			Destination: *output,
...
func runBackupAcceptArchiveLoss(args []string) error {
	...
	defer archive.Close()
	result, err := edgeapp.NewService(archive).Dispatch(
		context.Background(), edgeapp.LocalCLIActor(), edgeapp.AcceptRestoredArchiveLoss{
			EdgeNodeID: *edgeNodeID, LedgerEpoch: *ledgerEpoch,
			ConfirmedEdgeID: *confirmEdgeID, Reason: *reason,
		},
	)
	if err != nil {
		return err
...
func runAccount(args []string) error {
	if len(args) == 0 {
		return errors.New("usage: iotkit-edge account <bootstrap|recover> [options]")
	}
	switch args[0] {
	case "bootstrap":
...
func runAccountBootstrap(args []string) error {
	...
	result, err := service.DispatchAccount(
		...
		edgeapp.CreateInitialSystemAdmin{
			LoginID:     *loginID,
...
func runAccountRecover(args []string) error {
	...
	result, err := service.DispatchAccount(
		context.Background(),
		edgeapp.LocalCLIActor(),
		edgeapp.RecoverSystemAdminPassword{LoginID: *loginID, Password: password},
	)
	if err != nil {
		return err
	}
...
func runServe(args []string) error {
	...
	httpListen := flags.String("http-listen", "127.0.0.1:8080", "private Edge HTTP listen address")
	...
	outputClientID := flags.String("output-client-id", "iotkit-edge-output", "external MQTT client ID")
	...
	if !*allowInsecure {
		...
		tlsConfig, err = mqttedge.LoadTLSConfig(mqttedge.TrustMode(*trustMode), *caFile)
		if err != nil {
			return err
		}
	}
	var outputConfig *mqttedge.ClientConfig
	if *outputBrokerURL != "" {
		...
		if !*outputAllowInsecure {
			...
			outputTLS, err = mqttedge.LoadTLSConfig(
				mqttedge.TrustMode(*outputTrustMode), *outputCAFile,
			)
			if err != nil {
				return err
			}
		}
		outputConfig = &mqttedge.ClientConfig{
	...
	if err != nil {
		return fmt.Errorf("initialize Edge sessions: %w", err)
	}
	httpHandler, err := edgehttp.New(edgehttp.Config{
		Store:                 archive,
	...
	if err != nil {
		return fmt.Errorf("initialize Edge HTTP: %w", err)
	}
	...
	go func() {
		...
		if outputConfig != nil {
			errorsCh <- mqttedge.RunIngest(
				runCtx, config, mqttedge.Processor{Store: archive},
				archive, slog.Default(),
			)
			return
		}
		errorsCh <- mqttedge.Run(
			runCtx, config, mqttedge.Processor{Store: archive},
			archive, slog.Default(),
		)
	}()
	if outputConfig != nil {
		go func() {
			errorsCh <- mqttedge.RunOutput(runCtx, *outputConfig, archive, slog.Default())
		}()
	}
	runErr := <-errorsCh
	cancel()
...
func runMappingSet(args []string) error {
	...
	service, archive, err := openEdgeService(*dbPath)
	if err != nil {
		return err
	}
	defer archive.Close()
	result, err := service.Dispatch(
		context.Background(),
		edgeapp.LocalCLIActor(),
		edgeapp.PutSemanticMapping{Spec: spec},
	)
	if err != nil {
		return err
	}
...
func runMappingDeactivate(args []string) error {
	...
	result, err := service.Dispatch(
		context.Background(),
		edgeapp.LocalCLIActor(),
		edgeapp.DeactivateSemanticMapping{EdgeNodeID: *edgeNodeID, SeriesKey: *seriesKey},
	)
	if err != nil {
		return err
	}
...
func runRouteAdd(args []string) error {
	...
	operation := edgeapp.PutLegacyMQTTRoute{MappingID: *mappingID, Topic: *topic}
	...
	defer archive.Close()
	result, err := service.Dispatch(context.Background(), edgeapp.LocalCLIActor(), operation)
	if err != nil {
		return err
	}
...
func openEdgeService(dbPath string) (*edgeapp.Service, *store.Store, error) {
	archive, err := store.Open(dbPath)
	if err != nil {
		return nil, nil, err
	}
	return edgeapp.NewService(archive), archive, nil
}

func openAccountService(dbPath string) (*edgeapp.AccountService, *store.Store, error) {
	archive, err := store.Open(dbPath)
	if err != nil {
		return nil, nil, err
	}
	return edgeapp.NewAccountService(archive), archive, nil
}
