...
func (store *Store) RecordAuditEvent(ctx context.Context, event edgeapp.AuditEvent) error {
...
func (store *Store) DeactivateSemanticMapping(
	...
	if errors.Is(err, sql.ErrNoRows) {
		return noMapping, edgeapp.ErrNotFound
	}
	if err != nil {
		return noMapping, err
	}
...
func (store *Store) ListAuditEvents(ctx context.Context, limit int) ([]edgeapp.AuditEvent, error) {
	...
	events := make([]edgeapp.AuditEvent, 0)
	for rows.Next() {
		var event edgeapp.AuditEvent
