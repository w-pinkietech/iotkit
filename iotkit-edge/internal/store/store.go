...
var (
	...
	ErrArchiveRecoveryRequired = errors.New("restored IoTKit Edge is missing Edge Node records and requires an explicit recovery decision")
)
...
func (store *Store) initialize(configuredEdgeID string) error {
	...
	return store.validateEdgeIdentity(context.Background())
}
...
func (store *Store) rejectLegacyGatewaySchema() error {
	for _, table := range []string{"raw_records", "accepted_cursors"} {
		...
		if legacyColumns > 0 {
			return errors.New("unsupported pre-release Edge database; recreate it")
		}
	}
	return nil
}
...
func checkRevisionPrecondition(precondition edgeapp.RevisionPrecondition, exists bool, revision int64) error {
