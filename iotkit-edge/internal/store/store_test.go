...
func putSemanticMappingForTest(
	...
	return store.ApplySemanticMapping(ctx, edgeapp.LocalCLIActor(), spec, edgeapp.RevisionPrecondition{})
}

func putMQTTRouteForTest(store *Store, ctx context.Context, mappingID, topic string) (MQTTRoute, error) {
	route, err := store.ApplyLegacyMQTTRoute(ctx, edgeapp.LocalCLIActor(), mappingID, topic)
	if err != nil {
...
func TestOpenRejectsLegacyGatewayIdentitySchemaBeforeCreatingTables(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			path := filepath.Join(t.TempDir(), "legacy-edge.db")
			...
			if err == nil || !strings.Contains(err.Error(), "unsupported pre-release Edge database; recreate it") {
				t.Fatalf("Open error = %v, want unsupported pre-release Edge database", err)
			}
...
func TestOpenWithEdgeIDPersistsDeploymentIdentityAndRejectsMismatch(t *testing.T) {
	path := filepath.Join(t.TempDir(), "edge.db")
	const edgeID = "edge-0123456789abcdef0123456789abcdef"
	archive, err := OpenWithEdgeID(path, edgeID)
	if err != nil {
		t.Fatal(err)
	}
	var stored string
	...
	if stored != edgeID {
		t.Fatalf("stored IoTKit Edge ID=%q", stored)
	}
	if err := archive.Close(); err != nil {
		t.Fatal(err)
	}
	if _, err := OpenWithEdgeID(
		path,
		"edge-ffffffffffffffffffffffffffffffff",
	); err == nil {
		t.Fatal("existing database accepted a different IoTKit Edge ID")
	}
}

func TestOpenWithEdgeIDPersistsIdentityInTheEdgeMetaMigrationTransaction(
	t *testing.T,
) {
	path := filepath.Join(t.TempDir(), "edge.db")
	db, err := sql.Open("sqlite", path)
	...
	const edgeID = "edge-0123456789abcdef0123456789abcdef"
	if archive, err := OpenWithEdgeID(path, edgeID); err == nil {
		_ = archive.Close()
	...
	if storedEdgeID != edgeID {
		t.Fatalf("IoTKit Edge ID after interrupted migration=%q, want %q", storedEdgeID, edgeID)
	}
}
...
func TestPutSemanticMappingCapturesEveryExistingEpochCursor(t *testing.T) {
	...
	acceptEpoch(t, store, "edge-node-02", "epoch-other-edge-node", 1, 1)
