...
func TestCreateInitialEdgeAccountIsAtomicAndAudited(t *testing.T) {
	store := openTestStore(t)
	ctx := context.Background()
	...
	account, err := store.CreateEdgeAccount(ctx, edgeapp.LocalCLIActor(), provision)
	if err != nil {
		t.Fatal(err)
	}
	if account.AccountRef == "" || account.Revision != 1 ||
		account.Role != edgeapp.AccountRoleSystemAdmin ||
		account.State != edgeapp.AccountStateActive {
		t.Fatalf("created account = %#v", account)
	}
	if _, err := store.CreateEdgeAccount(
		ctx,
		edgeapp.LocalCLIActor(),
		provision,
	); !errors.Is(err, edgeapp.ErrAlreadyOwned) {
...
func TestDisableEdgeAccountProtectsLastSystemAdminAndRevokesSessions(t *testing.T) {
	store := openTestStore(t)
	ctx := context.Background()
	owner := createEdgeAccountForTest(
		t,
		store,
		edgeapp.AccountRoleSystemAdmin,
		"edge.owner",
	)
	...
	if _, err := store.DisableEdgeAccount(
		ctx,
		edgeapp.AccountActor(owner.AccountRef, edgeapp.AccountRoleSystemAdmin),
		owner.AccountRef,
		owner.Revision,
	); !errors.Is(err, edgeapp.ErrLastSystemAdmin) {
		t.Fatalf("last system admin disable error = %v", err)
	}
	second := createEdgeAccountForTest(
		t,
		store,
		edgeapp.AccountRoleSystemAdmin,
		"edge.backup",
	)
	disabled, err := store.DisableEdgeAccount(
		ctx,
		edgeapp.AccountActor(second.AccountRef, edgeapp.AccountRoleSystemAdmin),
		owner.AccountRef,
		owner.Revision,
	)
	if err != nil {
	...
	if disabled.State != edgeapp.AccountStateDisabled || disabled.Revision != owner.Revision+1 {
...
func TestReplaceEdgeAccountPasswordIsRevisionProtectedAndRevokesSessions(t *testing.T) {
	store := openTestStore(t)
	ctx := context.Background()
	owner := createEdgeAccountForTest(
		t,
	...
	if _, err := store.ReplaceEdgeAccountPassword(
		ctx,
		edgeapp.AccountActor(owner.AccountRef, edgeapp.AccountRoleSystemAdmin),
		owner.AccountRef,
		"$argon2id$new",
		true,
		owner.Revision+1,
	); !errors.Is(err, edgeapp.ErrRevisionMismatch) {
		t.Fatalf("password revision error = %v, want ErrRevisionMismatch", err)
	}
	updated, err := store.ReplaceEdgeAccountPassword(
		ctx,
		edgeapp.AccountActor(owner.AccountRef, edgeapp.AccountRoleSystemAdmin),
		owner.AccountRef,
		"$argon2id$new",
...
func TestAccountAuditSnapshotsActorIdentityWithoutCredential(t *testing.T) {
	store := openTestStore(t)
	ctx := context.Background()
	actorAccount := createEdgeAccountForTest(
		t,
		store,
		edgeapp.AccountRoleSystemAdmin,
		"edge.owner",
	)
	target := createEdgeAccountForTest(
		t,
		store,
		edgeapp.AccountRoleViewer,
		"operator",
	)
	if _, err := store.UpdateEdgeAccount(
		ctx,
		edgeapp.AccountActor(actorAccount.AccountRef, edgeapp.AccountRoleSystemAdmin),
		target.AccountRef,
		"設備担当者",
		edgeapp.AccountRoleAdmin,
		target.Revision,
	); err != nil {
		t.Fatal(err)
	}
...
func createEdgeAccountForTest(
	...
	t.Helper()
	account, err := store.CreateEdgeAccount(
		context.Background(),
		edgeapp.LocalCLIActor(),
