...
func TestActivateEdgeNodeRequiresAnAdministrator(t *testing.T) {
	...
	_, err := service.Dispatch(
		...
		ActivateEdgeNode{
			EdgeNodeRef: "edge_node_00000000000000000000000000000001",
	...
	if repository.activateEdgeNodeCalls != 0 {
		t.Fatalf("activation calls = %d, want 0", repository.activateEdgeNodeCalls)
	}
}

func TestAdministratorCanActivateAndListEdgeNodes(t *testing.T) {
	edgeNode := EdgeNode{
		EdgeNodeRef: "edge_node_00000000000000000000000000000001",
	...
	repository := &fakeRepository{edgeNodes: []EdgeNode{edgeNode}, edgeNode: edgeNode}
	service := NewService(repository)
	expected := int64(1)
	...
	result, err := service.Dispatch(
		...
		ActivateEdgeNode{
			EdgeNodeRef: edgeNode.EdgeNodeRef,
			Precondition: RevisionPrecondition{
				Expected: &expected,
			},
		},
	...
	if result.EdgeNode == nil || result.EdgeNode.EdgeNodeRef != edgeNode.EdgeNodeRef {
		t.Fatalf("result = %#v", result)
	}
	edgeNodes, err := service.ListEdgeNodes(context.Background())
	if err != nil || len(edgeNodes) != 1 || edgeNodes[0].EdgeNodeRef != edgeNode.EdgeNodeRef {
...
func TestMaintenanceOperationsRequireLocalCLIAndUseTypedRepositoryMethods(t *testing.T) {
	...
	if _, err := service.Dispatch(context.Background(), viewer, CreateEdgeBackup{
		Destination: "/backup/edge", Passphrase: "secret",
	}); !errors.Is(err, ErrForbidden) {
		t.Fatalf("network backup error=%v, want ErrForbidden", err)
	}
	result, err := service.Dispatch(
		context.Background(), LocalCLIActor(),
		CreateEdgeBackup{Destination: "/backup/edge", Passphrase: "secret"},
	)
	...
	accepted, err := service.Dispatch(
		...
		AcceptRestoredArchiveLoss{
			...
			ConfirmedEdgeID: "edge-01", Reason: "verified unavailable archive",
		},
	)
...
func (repository *fakeRepository) ListEdgeNodes(context.Context) ([]EdgeNode, error) {
	return repository.edgeNodes, nil
}

func (repository *fakeRepository) RequestEdgeNodeActivation(
	context.Context,
	Actor,
	string,
	RevisionPrecondition,
) (EdgeNode, error) {
	repository.activateEdgeNodeCalls++
	return repository.edgeNode, nil
}
