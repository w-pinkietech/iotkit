...
var (
	ErrNotFound         = errors.New("Edge resource not found")
	ErrRevisionMismatch = errors.New("Edge resource revision mismatch")
	ErrForbidden        = errors.New("Edge operation is forbidden")
	ErrAlreadyOwned     = errors.New("Edge already has an owner")
...
func (actor Actor) Validate() error {
	if actor.Class != ActorLocalCLI && actor.Class != ActorSettingsSession &&
		...
		return errors.New("unsupported Edge actor class")
	}
	if strings.TrimSpace(actor.Ref) == "" {
		return errors.New("Edge actor ref must not be empty")
	}
	if len(actor.Ref) > 128 {
		return errors.New("Edge actor ref must not exceed 128 bytes")
	}
	if strings.IndexFunc(actor.Ref, unicode.IsControl) >= 0 {
		return errors.New("Edge actor ref must not contain control characters")
	}
	if actor.Class == ActorAccount {
		...
		if !actor.Role.Valid() {
			return errors.New("Edge account actor role is invalid")
		}
	}
	return nil
}
...
const (
	EdgeNodeDiscovered   EdgeNodeState = "discovered"
	EdgeNodeActivating   EdgeNodeState = "activating"
	EdgeNodeActive       EdgeNodeState = "active"
	EdgeNodeRecoveryHold EdgeNodeState = "recovery_hold"
)

type EdgeNode struct {
	EdgeNodeRef      string        `json:"edge_node_ref"`
...
type LatestMeasurement struct {
	...
	EdgeReceivedAt int64           `json:"edge_received_at"`
}
