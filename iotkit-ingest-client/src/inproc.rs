...
/// IoTKit Edge NodeはJoinHandleでそれを監視する(fail-fast=計画2のSubmitError分離の消費)。
///
/// 設計不変則(計画レビュー裁定反映):
...
fn spawn_inproc_inner(
    ...
    let handle = tokio::spawn(async move {
        ...
        loop {
            // 1) 送信可能なら先頭を送る
            let ready =
                !spool.is_empty() && backoff_until.is_none_or(|t| tokio::time::Instant::now() >= t);
            if ready {
...
#[cfg(test)]
#[path = "../tests/unit/inproc_tests.rs"]
mod tests;
