...
async fn known_key_in_range_is_durable_and_auto_enables() {
    ...
    let (entries, events, readings): (i64, i64, i64) = db
        .with_conn_sync(|conn| {
            Ok((
                conn.query_row(
                ...
                .unwrap(),
                conn.query_row(
                    "SELECT COUNT(*) FROM readings WHERE quarantined=0",
                    [],
                    |r| r.get(0),
                )
                .unwrap(),
...
async fn unknown_key_materializes_quarantined_series_with_reason() {
    ...
    let (key, q, reason): (String, i64, Option<String>) = db
        .with_conn_sync(|conn| {
            Ok(conn
                .query_row(
...
async fn single_mode_none_and_zero_channel_share_one_series() {
    ...
    collector
        ...
        .await
        .unwrap();
    let (series, channel, readings): (i64, i32, i64) = db
        .with_conn_sync(|conn| {
            Ok((
                conn.query_row("SELECT COUNT(*) FROM series", [], |r| r.get(0))
                    .unwrap(),
                conn.query_row("SELECT channel_index FROM series", [], |r| r.get(0))
                    .unwrap(),
        ...
        .unwrap();
    assert_eq!(
        (series, channel, readings),
        (1, -1, 2),
        "series分裂しない(正準チャネル=-1)"
    );
}
...
async fn alias_routes_new_series_to_canonical_key() {
    ...
    let key: String = db
        .with_conn_sync(|conn| {
            Ok(conn
                .query_row("SELECT measurement_key FROM series", [], |r| r.get(0))
                .unwrap())
        })
        .unwrap();
...
async fn auto_enable_failure_produces_no_ack_and_retry_recovers_consistently() {
    ...
    let (dedup, entries, events, readings): (i64, i64, i64, i64) = db
        .with_conn_sync(|conn| {
            conn.execute_batch("DROP TRIGGER fail_enable;")?;
            Ok((
                conn.query_row("SELECT COUNT(*) FROM ingest_dedup", [], |r| r.get(0))
                    .unwrap(),
    ...
    assert!(matches!(ack.status,
        AckStatus::Accepted { ref items }
        if matches!(items[0], ItemStatus::Stored { disposition: Disposition::Durable, .. })));
    let (entries2, events2): (i64, i64) = db
        .with_conn_sync(|conn| {
            Ok((
                conn.query_row("SELECT COUNT(*) FROM registry_entries", [], |r| r.get(0))
                    .unwrap(),
