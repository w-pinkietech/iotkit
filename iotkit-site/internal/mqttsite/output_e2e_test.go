...
func runOutputAdapterBrokerJourney(t *testing.T) {
	...
	archive, err := store.Open(filepath.Join(controlDir, "site.db"))
	if err != nil {
		t.Fatal(err)
	}
	defer archive.Close()
	...
	go func() {
		runErrors <- RunOutput(ctx, ClientConfig{
			...
			Username: "site-output", Password: outputPassword, AllowInsecure: true,
...
func prepareOutputRoutes(t *testing.T, archive *store.Store) (string, string, string) {
	...
	edges, err := archive.ListEdges(ctx)
	if err != nil || len(edges) != 1 {
		t.Fatalf("discovered edges = %#v, %v", edges, err)
	}
