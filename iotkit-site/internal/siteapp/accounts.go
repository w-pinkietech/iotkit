...
type AccountRepository interface {
	CreateSiteAccount(context.Context, Actor, AccountProvision) (Account, error)
	GetSiteAccount(context.Context, string) (Account, error)
	GetSiteAccountByLoginID(context.Context, string) (Account, error)
	GetSiteAccountCredential(context.Context, string) (AccountCredential, error)
	ListSiteAccounts(context.Context) ([]Account, error)
	UpdateSiteAccount(
		context.Context,
		Actor,
		string,
	...
	DisableSiteAccount(context.Context, Actor, string, int64) (Account, error)
	ReplaceSiteAccountPassword(
		context.Context,
		Actor,
		string,
		string,
...
func (service *AccountService) DispatchAccount(
	...
	if service == nil || service.repository == nil {
		return noResult, errors.New("Site account repository is nil")
	}
	...
	case CreateInitialSystemAdmin:
		...
		account, err := service.repository.CreateSiteAccount(ctx, actor, provision)
		if err != nil {
			return noResult, err
		}
	...
	case DisableAccount:
		...
		account, err := service.repository.DisableSiteAccount(
			ctx,
			actor,
	...
	case UpdateAccount:
		...
		if !operation.Role.Valid() {
			return noResult, errors.New("unsupported Site account role")
		}
		account, err := service.repository.UpdateSiteAccount(
			ctx,
			actor,
			operation.AccountRef,
	...
	case ResetAccountPassword:
		...
		passwordPHC, err := siteauth.HashPassword(operation.TemporaryPassword)
		if err != nil {
			return noResult, err
		}
		account, err := service.repository.ReplaceSiteAccountPassword(
			ctx,
			actor,
			operation.AccountRef,
			passwordPHC,
	...
	case RecoverSystemAdminPassword:
		...
		normalizedLoginID, err := siteauth.NormalizeLoginID(operation.LoginID)
		if err != nil {
			return noResult, err
		}
		account, err := service.repository.GetSiteAccountByLoginID(ctx, normalizedLoginID)
		if err != nil {
			return noResult, err
		}
		...
		passwordPHC, err := siteauth.HashPassword(operation.Password)
		if err != nil {
			return noResult, err
		}
		account, err = service.repository.ReplaceSiteAccountPassword(
	...
	case ChangeOwnPassword:
		...
		credential, err := service.repository.GetSiteAccountCredential(ctx, actor.Ref)
		if err != nil {
			return noResult, err
		}
		ok, _, err := siteauth.VerifyPassword(
		...
		passwordPHC, err := siteauth.HashPassword(operation.NewPassword)
		if err != nil {
			return noResult, err
		}
		account, err := service.repository.ReplaceSiteAccountPassword(
		...
		return AccountResult{Account: &account}, nil
	default:
		return noResult, errors.New("unsupported Site account operation")
	}
}
...
func (service *AccountService) ListAccounts(
	...
	if service == nil || service.repository == nil {
		return nil, errors.New("Site account repository is nil")
	}
	if err := actor.Validate(); err != nil {
		return nil, err
	}
	...
	return service.repository.ListSiteAccounts(ctx)
}
...
func makeProvision(
	...
	normalizedLoginID, err := siteauth.NormalizeLoginID(loginID)
	if err != nil {
	...
	if !role.Valid() {
		return AccountProvision{}, errors.New("unsupported Site account role")
	}
	passwordPHC, err := siteauth.HashPassword(password)
	if err != nil {
		return AccountProvision{}, err
	}
