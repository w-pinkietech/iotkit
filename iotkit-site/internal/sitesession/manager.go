...
var (
	...
	ErrUnauthenticated    = errors.New("Site session is not authenticated")
	ErrBusy               = errors.New("Site authentication is busy")
)
...
func NewManager(archive *store.Store, options Options) (*Manager, error) {
	if archive == nil {
		return nil, errors.New("Site session store is nil")
	}
	now := options.Now
	if now == nil {
		now = time.Now
	...
	dummyPHC, err := siteauth.HashPassword("dummy-authentication-password")
	if err != nil {
		return nil, err
	}
	return &Manager{
...
func (manager *Manager) Login(
	...
	normalized, normalizationErr := siteauth.NormalizeLoginID(loginID)
	...
	ok, _, verifyErr := siteauth.VerifyPassword(encoded, password)
	...
	if !valid {
		...
		_ = manager.store.RecordAuditEvent(ctx, siteapp.AuditEvent{
			OccurredAt:  now.UnixMilli(),
			ActorClass:  siteapp.ActorSystem,
	...
	if err := manager.store.CreateSessionWithAudit(ctx, record, siteapp.AuditEvent{
		OccurredAt:  issuedAt,
		ActorClass:  siteapp.ActorAccount,
...
func principalFromAccount(account store.AccountRecord, sessionRef string) Principal {
	return Principal{
		...
		Role:               siteapp.AccountRole(account.Role),
...
func randomSecret(size int) (string, error) {
	...
	if _, err := rand.Read(value); err != nil {
		return "", errors.New("generate Site session secret")
	}
...
func randomReference(prefix string) (string, error) {
	value := make([]byte, 16)
	if _, err := rand.Read(value); err != nil {
		return "", errors.New("generate Site session reference")
	}
