...
type BackupManifest = siteapp.BackupManifest
...
func (store *Store) ApplyEncryptedBackup(
	...
	if actor.Class != siteapp.ActorLocalCLI {
		return empty, siteapp.ErrForbidden
	}
	...
	snapshotFile, err := os.CreateTemp(directory, ".iotkit-site-snapshot-*")
	if err != nil {
		return empty, fmt.Errorf("create Site snapshot staging file: %w", err)
	}
	...
	if _, err := tx.ExecContext(ctx, `
		INSERT INTO site_backup_events(
	...
	`, manifest.BackupID, manifest.CreatedAt, filepath.Base(destination),
		...
		_ = os.Remove(destination)
		return empty, fmt.Errorf("record completed Site backup: %w", err)
	}
	for _, cursor := range manifest.Cursors {
		if _, err := tx.ExecContext(ctx, `
			INSERT INTO site_backup_cursors(
		...
		`, manifest.BackupID, cursor.EdgeNodeID,
			...
			_ = os.Remove(destination)
			return empty, fmt.Errorf("record completed Site backup cursor: %w", err)
		}
	}
	...
	if err := insertAuditEventTx(ctx, tx, siteapp.AuditEvent{
		...
		Operation: "site_backup.create", ResourceRef: manifest.BackupID,
		Outcome: auditOutcomeSuccess, Summary: summary,
	}); err != nil {
		_ = os.Remove(destination)
		return empty, fmt.Errorf("audit completed Site backup: %w", err)
	}
	if err := tx.Commit(); err != nil {
		_ = os.Remove(destination)
		return empty, fmt.Errorf("commit completed Site backup: %w", err)
	}
	return manifest, nil
}
...
func RestoreEncryptedBackup(
	...
	directory := filepath.Dir(destination)
	payload, err := os.CreateTemp(directory, ".iotkit-site-restore-payload-*")
	if err != nil {
	...
	database, err := os.CreateTemp(directory, ".iotkit-site-restored-db-*")
	if err != nil {
		_ = payload.Close()
		return empty, err
	}
	...
	if _, err := io.Copy(io.MultiWriter(database, hash), payload); err != nil {
		_ = payload.Close()
		_ = database.Close()
		return empty, fmt.Errorf("extract Site database: %w", err)
	}
	...
	if got := hex.EncodeToString(hash.Sum(nil)); !strings.EqualFold(got, manifest.DatabaseSHA256) {
		return empty, errors.New("Site backup database checksum does not match its manifest")
	}
...
func encryptBackupContainer(
	...
	if len(manifestJSON) > maxManifestSize {
		return errors.New("Site backup manifest is too large")
	}
	...
	directory := filepath.Dir(destination)
	output, err := os.CreateTemp(directory, ".iotkit-site-backup-*")
	if err != nil {
...
func decryptBackupContainer(source string, destination io.Writer, passphrase string) error {
	...
	for sequence := uint64(0); ; sequence++ {
		...
		if err := binary.Read(input, binary.BigEndian, &ciphertextLength); err != nil {
			return errors.New("Site backup is truncated before its final authenticated chunk")
		}
		maximum := uint32(header.ChunkSize + 1 + aead.Overhead())
		if ciphertextLength < uint32(1+aead.Overhead()) || ciphertextLength > maximum {
			return errors.New("invalid encrypted Site backup chunk")
		}
		ciphertext := make([]byte, ciphertextLength)
		if _, err := io.ReadFull(input, ciphertext); err != nil {
			return errors.New("truncated encrypted Site backup chunk")
		}
		plain, err := aead.Open(nil, backupNonce(noncePrefix, sequence), ciphertext, backupAAD(headerDigest, sequence))
		if err != nil {
			return errors.New("Site backup authentication failed; the passphrase is wrong or the backup was changed")
		}
		if len(plain) == 0 || (plain[0] != 0 && plain[0] != 1) {
			return errors.New("invalid authenticated Site backup chunk")
		}
		if plain[0] == 1 {
			if len(plain) != 1 {
				return errors.New("invalid Site backup final chunk")
			}
			var trailing [1]byte
			if count, err := input.Read(trailing[:]); err != io.EOF || count != 0 {
				return errors.New("Site backup has unauthenticated trailing data")
			}
			return nil
		}
...
func validateBackupHeader(header backupContainerHeader) ([]byte, []byte, error) {
	if header.FormatVersion != backupFormatVersion || header.KDF != "argon2id" || header.Cipher != "xchacha20-poly1305" {
		return nil, nil, errors.New("unsupported Site backup cryptographic format")
	}
	if header.KDFTime < 1 || header.KDFTime > 10 || header.KDFMemoryKiB < 16*1024 || header.KDFMemoryKiB > 256*1024 || header.KDFThreads < 1 || header.KDFThreads > 16 {
		return nil, nil, errors.New("unsafe Site backup key-derivation parameters")
	}
	if header.ChunkSize < 4096 || header.ChunkSize > 4*1024*1024 {
		return nil, nil, errors.New("invalid Site backup chunk size")
	}
	salt, err := base64.RawStdEncoding.DecodeString(header.Salt)
	if err != nil || len(salt) != 16 {
		return nil, nil, errors.New("invalid Site backup salt")
	}
	noncePrefix, err := base64.RawStdEncoding.DecodeString(header.NoncePrefix)
	if err != nil || len(noncePrefix) != 16 {
		return nil, nil, errors.New("invalid Site backup nonce")
	}
...
func readBackupManifest(payload io.Reader) (BackupManifest, error) {
	...
	if err := binary.Read(payload, binary.BigEndian, &length); err != nil || length == 0 || length > maxManifestSize {
		return BackupManifest{}, errors.New("invalid Site backup manifest length")
	}
	encoded := make([]byte, length)
	if _, err := io.ReadFull(payload, encoded); err != nil {
		return BackupManifest{}, errors.New("truncated Site backup manifest")
	}
	var manifest BackupManifest
	decoder := json.NewDecoder(bytes.NewReader(encoded))
	decoder.DisallowUnknownFields()
	if err := decoder.Decode(&manifest); err != nil {
		return BackupManifest{}, errors.New("invalid Site backup manifest")
	}
	if manifest.FormatVersion != backupFormatVersion || manifest.BackupID == "" || manifest.CreatedAt < 0 || manifest.SiteID == "" || manifest.SchemaVersion < 1 || manifest.RawRecordCount < 0 {
		return BackupManifest{}, errors.New("invalid Site backup manifest values")
	}
	decodedHash, err := hex.DecodeString(manifest.DatabaseSHA256)
	if err != nil || len(decodedHash) != sha256.Size {
		return BackupManifest{}, errors.New("invalid Site backup database checksum")
	}
	return manifest, nil
}
...
func validateRestoredSnapshot(ctx context.Context, path string, manifest BackupManifest) error {
	...
	if info.SiteID != manifest.SiteID || info.SchemaVersion != manifest.SchemaVersion || info.RawRecordCount != manifest.RawRecordCount {
		return errors.New("Site backup manifest does not match the restored database")
	}
	if len(info.Cursors) != len(manifest.Cursors) {
		return errors.New("Site backup cursor manifest does not match the restored database")
	}
...
func prepareRestoredDatabase(ctx context.Context, path string, manifest BackupManifest) error {
	...
	if err != nil {
		return fmt.Errorf("open restored Site database: %w", err)
	}
	closed := false
	defer func() {
		if !closed {
	...
	defer tx.Rollback()
	if _, err := tx.ExecContext(ctx, `
		UPDATE site_sessions SET revoked_at = ? WHERE revoked_at IS NULL
	`, now); err != nil {
		return err
	}
	if _, err := tx.ExecContext(ctx, `
		INSERT OR IGNORE INTO site_backup_events(
			backup_id, created_at, destination_name,
			database_sha256, raw_record_count
	...
	for _, cursor := range manifest.Cursors {
		if _, err := tx.ExecContext(ctx, `
			INSERT OR IGNORE INTO site_backup_cursors(
				backup_id, edge_node_id, ledger_epoch, accepted_through
			) VALUES(?, ?, ?, ?)
		`, manifest.BackupID, cursor.EdgeNodeID,
			cursor.LedgerEpoch, cursor.AcceptedThrough); err != nil {
	...
	if _, err := tx.ExecContext(ctx, `
		INSERT INTO site_restore_events(
			restore_id, backup_id, restored_at, backup_created_at, backup_site_id,
			backup_schema_version, backup_sha256
		) VALUES(?, ?, ?, ?, ?, ?, ?)
	`, restoreID, manifest.BackupID, now, manifest.CreatedAt, manifest.SiteID, manifest.SchemaVersion, manifest.DatabaseSHA256); err != nil {
		return err
	}
	for _, cursor := range manifest.Cursors {
		if _, err := tx.ExecContext(ctx, `
	...
	actor := siteapp.LocalCLIActor()
	if err := insertAuditEventTx(ctx, tx, siteapp.AuditEvent{
		OccurredAt: now, ActorClass: actor.Class, ActorRef: actor.Ref,
		Operation: "site_backup.restore", ResourceRef: restoreID,
...
func pendingRestoredCursorCheckTx(
	...
	err := tx.QueryRowContext(ctx, `
		...
		JOIN site_restore_events AS event
...
func (store *Store) ApplyRestoredArchiveLoss(
	...
	if confirmedSiteID != siteID {
		return errors.New("confirmed Site ID does not match this Site")
	}
	var restoreID string
	...
	if errors.Is(err, sql.ErrNoRows) {
		return errors.New("no restored archive-loss decision is pending for this Edge")
	}
	if err != nil {
		return err
	}
	...
	if changed, err := result.RowsAffected(); err != nil {
		...
		return errors.New("Edge is not in recovery hold for this restored cursor")
	}
	summary, err := json.Marshal(struct {
	...
	if err := insertAuditEventTx(ctx, tx, siteapp.AuditEvent{
		OccurredAt: now, ActorClass: actor.Class, ActorRef: actor.Ref,
		Operation: "site_restore.accept_archive_loss", ResourceRef: edgeNodeID,
		Outcome: auditOutcomeSuccess, Summary: summary,
	}); err != nil {
		return err
	}
