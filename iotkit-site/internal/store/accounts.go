...
func (store *Store) CreateSiteAccount(
	ctx context.Context,
	actor siteapp.Actor,
	provision siteapp.AccountProvision,
) (siteapp.Account, error) {
	var noAccount siteapp.Account
	...
	if provision.RequireUnowned {
		var count int
		if err := tx.QueryRowContext(ctx, "SELECT count(*) FROM site_accounts").Scan(&count); err != nil {
			return noAccount, err
		}
		if count != 0 {
			return noAccount, siteapp.ErrAlreadyOwned
		}
	}
	...
	account := siteapp.Account{
		...
		Role:               provision.Role,
		State:              siteapp.AccountStateActive,
	...
	if err != nil {
		if strings.Contains(err.Error(), "site_accounts.login_id_normalized") {
...
func (store *Store) GetSiteAccount(ctx context.Context, accountRef string) (siteapp.Account, error) {
	return scanSiteAccount(store.db.QueryRowContext(ctx, `
...
func (store *Store) GetSiteAccountByLoginID(
	ctx context.Context,
...
func (store *Store) GetSiteAccountCredential(
	ctx context.Context,
	accountRef string,
) (siteapp.AccountCredential, error) {
	var credential siteapp.AccountCredential
	var err error
	credential.Account, err = scanSiteAccount(store.db.QueryRowContext(ctx, `
		SELECT account_ref, login_id, display_name, role, state,
			must_change_password, revision, created_at, updated_at
		FROM site_accounts
		WHERE account_ref = ?
	`, accountRef))
	if err != nil {
		return siteapp.AccountCredential{}, err
	}
	if err := store.db.QueryRowContext(ctx, `
		SELECT password_phc FROM site_accounts WHERE account_ref = ?
...
func (store *Store) ListSiteAccounts(ctx context.Context) ([]siteapp.Account, error) {
	...
	defer rows.Close()
	accounts := make([]siteapp.Account, 0)
	for rows.Next() {
		account, err := scanSiteAccount(rows)
		if err != nil {
			return nil, err
		}
...
func (store *Store) UpdateSiteAccount(
	ctx context.Context,
	actor siteapp.Actor,
	accountRef string,
	displayName string,
	...
	account, err := scanSiteAccount(tx.QueryRowContext(ctx, `
		SELECT account_ref, login_id, display_name, role, state,
			must_change_password, revision, created_at, updated_at
		FROM site_accounts
		WHERE account_ref = ?
	...
	if account.Revision != expectedRevision {
		return noAccount, siteapp.ErrRevisionMismatch
	}
	if account.State != siteapp.AccountStateActive {
		return noAccount, siteapp.ErrNotFound
	}
	roleChanged := account.Role != role
	if account.Role == siteapp.AccountRoleSystemAdmin &&
		...
		if activeSystemAdmins <= 1 {
			return noAccount, siteapp.ErrLastSystemAdmin
		}
	}
	now := time.Now().UnixMilli()
...
func (store *Store) DisableSiteAccount(
	ctx context.Context,
	actor siteapp.Actor,
	accountRef string,
	expectedRevision int64,
	...
	now := time.Now().UnixMilli()
	account.State = siteapp.AccountStateDisabled
	account.Revision++
	account.UpdatedAt = now
	if _, err := tx.ExecContext(ctx, `
		UPDATE site_accounts
...
func (store *Store) ReplaceSiteAccountPassword(
	ctx context.Context,
	actor siteapp.Actor,
	accountRef string,
	passwordPHC string,
...
func scanSiteAccount(row rowScanner) (siteapp.Account, error) {
	var account siteapp.Account
	if err := row.Scan(
		&account.AccountRef,
		&account.LoginID,
		&account.DisplayName,
		...
		return siteapp.Account{}, siteapp.ErrNotFound
	} else if err != nil {
		return siteapp.Account{}, err
	}
	return account, nil
}
