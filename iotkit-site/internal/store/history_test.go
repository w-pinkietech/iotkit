...
func seedHistorySignal(t *testing.T, store *Store, edge, signalRef, seriesKey string) {
	t.Helper()
	...
	`, signalRef, edge, seriesKey, "device-01", int64(4_000), int64(1)); err != nil {
		t.Fatal(err)
	}
	if _, err := store.db.Exec(`
	...
	`, edge, seriesKey, "device-01", "temperature_c"); err != nil {
		t.Fatal(err)
	}
	if _, err := store.db.Exec(`
...
func seedHistoryRecord(
	...
	`, edge, epoch, seq, edge+":"+epoch, payload, receivedAt); err != nil {
		t.Fatal(err)
	}
}
