...
var siteIDPattern = regexp.MustCompile(`^site-[0-9a-f]{32}$`)
var outputRandomRead = rand.Read

func (store *Store) validateSiteIdentity(ctx context.Context) error {
	var count int
	var siteID string
	if err := store.db.QueryRowContext(ctx, `
		SELECT count(*), COALESCE(MAX(site_id), '') FROM site_meta
	`).Scan(&count, &siteID); err != nil {
		return err
	}
	if count != 1 || !siteIDPattern.MatchString(siteID) {
		return errors.New("Site identity metadata is missing or invalid")
	}
	return nil
}
...
func (store *Store) ActivateExportProfile(
	...
	displayName string,
	adapterID string,
) (siteapp.ExportProfile, error) {
	var noProfile siteapp.ExportProfile
	...
	sourceID, err := validatedSiteIDTx(ctx, tx)
	if err != nil {
		return noProfile, err
	}
	now := time.Now().UnixMilli()
	profileState := siteapp.ExportProfileActive
	if adapterID == "yokakit.mqtt.v1" {
		profileState = siteapp.ExportProfilePreparing
	}
	profile := siteapp.ExportProfile{
		ProfileID:            profileID,
...
func (store *Store) ListExportProfiles(
	...
	for rows.Next() {
		var profile siteapp.ExportProfile
...
func (store *Store) PreviewExportProfileActivation(
	ctx context.Context,
	adapterID string,
) (siteapp.ExportProfileActivationPreview, error) {
	var preview siteapp.ExportProfileActivationPreview
	...
	for rows.Next() {
		...
		var rule siteapp.OutputActivationRulePreview
		if err := rows.Scan(
...
func (store *Store) listProfileBindings(
	ctx context.Context,
	profileID string,
) ([]siteapp.OutputProfileRuleBinding, error) {
	rows, err := store.db.QueryContext(ctx, `
		...
		LEFT JOIN site_signals AS signal ON signal.signal_ref = rule.signal_ref
	...
	defer rows.Close()
	var bindings []siteapp.OutputProfileRuleBinding
	for rows.Next() {
...
func (store *Store) ConfigureYokaKitBooleanBinding(
	...
	var noBinding siteapp.OutputProfileRuleBinding
	if err := authorizeOutputMutation(actor); err != nil {
	...
	if errors.Is(err, sql.ErrNoRows) {
		return noBinding, siteapp.ErrNotFound
	}
	if err != nil {
		return noBinding, err
	}
	if profile.AdapterID != "yokakit.mqtt.v1" ||
		(profile.State != siteapp.ExportProfileActive &&
			profile.State != siteapp.ExportProfilePreparing) ||
		binding.State != siteapp.OutputBindingNeedsConfiguration ||
	...
	if binding.Revision != expectedRevision {
		return noBinding, siteapp.ErrRevisionMismatch
	}
	sourceID, err := validatedSiteIDTx(ctx, tx)
	if err != nil {
		return noBinding, err
...
func (store *Store) RequestExportProfileStop(
	...
	changed, _ := result.RowsAffected()
	if changed != 1 {
		return noProfile, siteapp.ErrRevisionMismatch
	}
	if _, err := tx.ExecContext(ctx, `
	...
	return noProfile, siteapp.ErrNotFound
}
...
func (store *Store) StartPreparedOutputBinding(
	...
	defer func() { _ = tx.Rollback() }()
	var binding siteapp.OutputProfileRuleBinding
	var profileState siteapp.ExportProfileState
	...
	if binding.State != siteapp.OutputBindingPrepared ||
		(profileState != siteapp.ExportProfilePreparing &&
			profileState != siteapp.ExportProfileActive) {
...
func (store *Store) GetOutputBindingPublication(
	ctx context.Context,
	bindingID string,
) (siteapp.OutputPublicationPreview, error) {
	var noPreview siteapp.OutputPublicationPreview
	...
	if errors.Is(err, sql.ErrNoRows) {
		return noPreview, siteapp.ErrNotFound
	}
	if err != nil {
...
) siteapp.OutputPublicationPreview {
	return siteapp.OutputPublicationPreview{
		BindingID:  bindingID,
...
func createProfileBindingTx(
	...
	now := time.Now().UnixMilli()
	binding := siteapp.OutputProfileRuleBinding{
		BindingID:       bindingID,
	...
	case "yokakit.mqtt.v1":
		...
		case semantics.KindBoolean:
			binding.State = siteapp.OutputBindingNeedsConfiguration
		case semantics.KindNumeric:
			binding.State = siteapp.OutputBindingIneligible
	...
	`, routeID, ruleID, profile.AdapterID, descriptor.ConfigSchemaVersion,
		[]byte(config), binding.State == siteapp.OutputBindingActive,
...
func findOrCreateOutputSignalIdentityTx(
	...
	if err == nil {
		if storedSourceID != sourceID {
			return "", "", errors.New(
				"output signal identity belongs to a different Site",
			)
		}
...
func validatedSiteIDTx(ctx context.Context, tx *sql.Tx) (string, error) {
	var count int
	var siteID string
...
func authorizeOutputMutation(actor siteapp.Actor) error {
	...
	if actor.Class == siteapp.ActorAccount &&
		(actor.Role == siteapp.AccountRoleAdmin ||
			actor.Role == siteapp.AccountRoleSystemAdmin) {
		return nil
	}
