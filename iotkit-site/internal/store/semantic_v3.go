...
func (store *Store) GetSemanticConfiguration(
	...
	`, signalRef).Scan(
		...
		if lookupErr := store.db.QueryRowContext(ctx, `
			SELECT EXISTS(SELECT 1 FROM site_signals WHERE signal_ref = ?)
		`, signalRef).Scan(&exists); lookupErr != nil {
			return semantics.Configuration{}, lookupErr
		}
		if exists == 0 {
			return semantics.Configuration{}, siteapp.ErrNotFound
		}
...
func activeSemanticRuleForUpdateV3(
	...
	if errors.Is(err, sql.ErrNoRows) {
		return semantics.Rule{}, "", 0, siteapp.ErrNotFound
	}
	if err != nil {
...
func auditSemanticRuleV3(
	...
	return insertAuditEventTx(ctx, tx, siteapp.AuditEvent{
...
func (store *Store) RequestSemanticCounterReset(
	...
	`, ruleID).Scan(&edgeNodeID, &kind); errors.Is(err, sql.ErrNoRows) {
		return noReset, siteapp.ErrNotFound
	} else if err != nil {
		return noReset, err
	}
...
func (store *Store) applySemanticCounterResetV3(
	...
	if err := insertAuditEventTx(ctx, tx, siteapp.AuditEvent{
		OccurredAt: now, ActorClass: siteapp.ActorSystem,
