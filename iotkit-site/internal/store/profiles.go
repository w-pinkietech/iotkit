...
func (store *Store) UpdateDeviceProfile(
...
) (siteapp.DeviceProfile, error) {
	var noProfile siteapp.DeviceProfile
	...
	if err := tx.QueryRowContext(ctx, `
		SELECT edge_node_id, system_id FROM site_devices WHERE device_ref = ?
...
func (store *Store) UpdateSignalProfile(
	...
	precondition siteapp.RevisionPrecondition,
) (siteapp.SignalProfile, error) {
	var noProfile siteapp.SignalProfile
	if err := actor.Validate(); err != nil {
		return noProfile, err
	}
	if err := input.Validate(); err != nil {
	...
	if err := tx.QueryRowContext(ctx, `
		SELECT edge_node_id, series_key FROM site_signals WHERE signal_ref = ?
