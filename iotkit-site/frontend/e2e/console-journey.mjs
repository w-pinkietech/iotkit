...
const siteURL = process.env.IOTKIT_SITE_E2E_URL;
const password = process.env.IOTKIT_SITE_E2E_PASSWORD;
if (!siteURL || !password) {
  throw new Error("IOTKIT_SITE_E2E_URL and IOTKIT_SITE_E2E_PASSWORD are required");
}
...
try {
  ...
  await devtools.navigate(`${siteURL}/status`, "/login");
  assert(
  ...
  await devtools.navigate(`${siteURL}/equipment`, "/equipment");
  ...
  assert(inactiveEdgePath?.startsWith("/equipment/edges/"), "inactive Edge detail link was not found");
  ...
  await devtools.waitForExpression(
    ...
    "Edge activation request",
  );

  await devtools.navigate(`${siteURL}/sensors`, "/sensors");
  ...
  await devtools.navigate(`${siteURL}/accounts`, "/accounts");
  ...
  console.log("Site Console browser journey passed");
} catch (error) {
