...
pub enum AckStatus {
    /// Processing was committed at the gateway's acknowledgement durability point.
    ///
...
/// at the gateway but withheld from downstream delivery.
...
pub enum QuarantineReason {
    ...
    /// corrected.
    UndeclaredChannel,
    /// The subject itself is currently in the gateway's quarantined state.
    ///
...
pub enum Disposition {
    /// The gateway now owns the retained data at the acknowledgement durability point.
    ///
    ...
    Staged,
    /// The observation is stored and visible at the gateway but isolated from delivery.
    ///
    /// The gateway retains it for operator resolution and withholds it from
