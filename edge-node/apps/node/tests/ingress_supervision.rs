...
use iotkit_edge_node::ingress::{IngressBindFuture, IngressComposition};
use iotkit_edge_node::network_authority::{NetworkAuthorityError, require_network_authority};
        ...
        iotkit_core_ops::dispatch(
            ...
            iotkit_core_ops::DispatchRequest {
                ...
                params: json!({
                    ...
                    "interface": interface,
                    "local_ingress_cidrs": [local_ingress_cidr],
...
const TEST_LOCAL_INTERFACE: &str = "plan6-test-local";
const TEST_LOCAL_CIDR: &str = "10.42.0.0/24";
...
impl IngressComposition for DeterministicIngressComposition {
    fn exposure(&self, interface: &str) -> Result<ExposureSnapshot, ListenerError> {
        if interface != TEST_LOCAL_INTERFACE {
...
async fn start_tls_supervisor() -> (RunningTlsSupervisor, TestTlsGeneration) {
    ...
    own(&db);
    db.with_conn_sync(|conn| {
        iotkit_edge_node::api::tls::ensure_tls_material(conn, dir.path())
            .map(|_| ())
            .map_err(|error| {
                iotkit_core_storage::StorageError::Sqlite(rusqlite::Error::ToSqlConversionFailure(
                    Box::new(error),
    ...
    let interface = TEST_LOCAL_INTERFACE.to_owned();
    let local_ingress_cidr = TEST_LOCAL_CIDR.to_owned();
    ...
    configure_tls_ingress(&db, logical_bind, &interface, &local_ingress_cidr);
    ...
    let task = iotkit_edge_node::ingress::spawn_ingress_supervisor_serving_with_composition(
        db.clone(),
...
async fn restored_local_recovery_drains_the_edge_ingress_listener() {
    ...
    let interface = TEST_LOCAL_INTERFACE;
    let local_ingress_cidr = TEST_LOCAL_CIDR;
    configure_private_ingress(&db, bind, interface, local_ingress_cidr);
    ...
    let health = Arc::new(Mutex::new(HealthState::new(90)));
    let composition = Arc::new(DeterministicIngressComposition::default());
    let supervisor = iotkit_edge_node::ingress::spawn_ingress_supervisor_serving_with_composition(
        db.clone(),
        dir.path().to_path_buf(),
        health.clone(),
        Duration::from_millis(1),
    ...
    assert!(
        ...
        "recovery authority must drain the Edge Node listener, not return an ingest auth response"
    );
    ...
    db.with_conn_sync(|conn| {
        conn.execute(
            "UPDATE ingress_listener_config SET enabled=1,desired_generation=1,
             ...
             local_ingress_cidrs='[\"192.168.1.0/24\"]',mode='tls',
