...
async fn fresh_database_never_starts_a_control_listener() {
    ...
    assert!(matches!(
        result,
        Err(iotkit_edge_node::api::ApiError::NotReady("unowned"))
    ));
}
...
#[tokio::test]
async fn bound_control_listener_drains_when_tls_partial_state_appears() {
    let _ = rustls::crypto::ring::default_provider().install_default();
    let dir = tempfile::tempdir().unwrap();
    let db = iotkit_core_storage::init_db(&dir.path().join("owned.db"), &all_migrations()).unwrap();
    let health = Arc::new(Mutex::new(HealthState::new(90)));
...
fn private_source_guard_accepts_only_private_or_link_local_sources() {
    use iotkit_edge_node::api::guard::is_private_source;
