...
fn certificate_params() -> Result<CertificateParams, rcgen::Error> {
    ...
    params
        .distinguished_name
        .push(DnType::CommonName, "iotkit-edge-node");
    params.not_after = time::OffsetDateTime::now_utc() + time::Duration::days(365 * 100);
    Ok(params)
}

fn subject_alt_names() -> Vec<String> {
    let mut names = vec!["iotkit-edge-node.local".to_string()];
    if let Some(hostname) = hostname() {
        let hostname = hostname.trim();
        if !hostname.is_empty() && hostname != "iotkit-edge-node.local" {
...
#[cfg(test)]
#[path = "../../tests/unit/api/tls_tests.rs"]
mod tests;
