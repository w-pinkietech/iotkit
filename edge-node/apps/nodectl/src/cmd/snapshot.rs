...
fn require_exhaustively_pristine_target(conn: &Connection) -> AppResult<()> {
    ...
    let canonical_ingress: bool = conn.query_row(
        "SELECT EXISTS(SELECT 1 FROM ingress_listener_config
         WHERE id=1 AND desired_generation=0 AND applied_generation=0 AND enabled=0
           ...
           AND local_ingress_cidrs='[]' AND mode='tls'
           ...
           AND applied_local_ingress_cidrs IS NULL AND applied_mode IS NULL
    ...
    let canonical_edge_node_activation: bool = conn.query_row(
        "SELECT EXISTS(SELECT 1 FROM edge_node_activation
         WHERE singleton = 1
    ...
    let edge_node_activation_rows: i64 =
        conn.query_row("SELECT COUNT(*) FROM edge_node_activation", [], |row| {
            row.get(0)
        })?;
    if !canonical_edge_node_activation || edge_node_activation_rows != 1 {
        return Err(
            "restore target is not empty: Edge Node activation contains enrollment state".into(),
        );
    }
    ...
    let mut stmt = conn.prepare(
        "SELECT name FROM sqlite_schema
         WHERE type = 'table'
           ...
           AND name NOT IN ('_schema_version', '_iotkit_edge_format', 'auth_state', 'device_flow_classes', 'device_capacity', 'ingress_listener_config', 'ingest_dedup_maintenance', 'ledger_meta', 'edge_node_activation')
         ORDER BY name",
    )?;
    let tables = stmt
