...
func TestActivationContractsRejectMalformedIdentityVersionAndBoundary(t *testing.T) {
	...
	request = validActivationRequest()
	request.EdgeID = "edge-short"
	if _, err := request.Encode(); err == nil {
		t.Fatal("short IoTKit Edge ID was accepted")
	}
	request = validActivationRequest()
	request.EdgeNodeID = "edge-node/other"
	if _, err := request.Encode(); err == nil {
		t.Fatal("unsafe EdgeNode ID was accepted")
	}
	request = validActivationRequest()
...
func TestSharedActivationFixturesMatchTheEdgeContract(t *testing.T) {
	...
	for _, name := range []string{
		"activation-request-wrong-edge-node.json",
