...
func runEdge(
	...
	options.SetOnConnectHandler(func(client mqtt.Client) {
		...
		if err == nil {
			logger.Info("IoTKit Edge subscribed", "topics", []string{
				recordsTopicFilter,
		...
		case <-ticker.C:
			if activationQueue, ok := queue.(activationCommandQueue); ok {
				if err := publishPendingActivationCommands(
					...
					logger.Error("MQTT Edge Node activation command failed", "error", err)
				}
			}
			convergeEdge(ctx, queue, logger)
			if publishOutputs {
...
func convergeEdge(ctx context.Context, queue ExportQueue, logger *slog.Logger) {
	if ctx.Err() != nil {
		return
	}
...
func publishPendingActivationCommands(
	...
	if err != nil {
		return fmt.Errorf("list pending Edge Node activation commands: %w", err)
	}
	var publishErrors []error
	for _, command := range pending {
		...
		if err := publish(command.Topic, 1, false, command.PayloadJSON); err != nil {
			...
			publishErrors = append(
				publishErrors,
				fmt.Errorf("publish Edge Node activation command %q: %w", command.ActivationID, err),
			)
			continue
		}
		if err := queue.MarkActivationCommandAttempt(
			...
			publishErrors = append(
				publishErrors,
				fmt.Errorf("record Edge Node activation attempt %q: %w", command.ActivationID, err),
			)
		}
	}
	return errors.Join(publishErrors...)
