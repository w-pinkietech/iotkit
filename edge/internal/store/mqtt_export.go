...
func (store *Store) ApplyLegacyMQTTRoute(
	...
	mappingID string,
	topic string,
) (edgeapp.LegacyMQTTRoute, error) {
	var noRoute edgeapp.LegacyMQTTRoute
	...
	return edgeapp.LegacyMQTTRoute{
...
func (store *Store) ListPendingMQTTExports(ctx context.Context, limit int) ([]PendingMQTTExport, error) {
	...
	rows, err := store.db.QueryContext(ctx, `
		WITH ranked_pending AS (
			...
			FROM output_outbox_v3 AS outbox
...
func (store *Store) MarkMQTTExportPublished(ctx context.Context, exportID string) error {
	...
	if rowsAffected == 0 {
		result, err = store.db.ExecContext(ctx, `
			UPDATE output_outbox_v3 SET published_at = COALESCE(published_at, ?)
			WHERE export_id = ?
		`, now, exportID)
		if err != nil {
			return err
