    ...
    for (path, hooks) in [
        ...
        (
            ...
            &[
                ...
                r#"class="history-chart-axis-title""#,
                r#"data-history-axis-time data-unix-ms="1735689601000""#,
                "横軸は受信日時、縦軸は値",
        ...
        if path == "/live" {
            ...
            assert_eq!(html.matches(r#"data-live-signal"#).count(), 1);
            assert!(html.contains(r#"data-rule-id="rule-02""#));
            assert!(!html.contains(r#"data-rule-id="rule-01""#));
            assert!(!html.contains(r#"data-rule-id="rule-03""#));
...
#[tokio::test]
async fn live_page_shows_one_dashboard_setup_message_without_active_cumulative_rules() {
    let app = router(WebConfig::test(), Arc::new(StubApplication::numeric_only()));
    let response = app
        .oneshot(
            Request::get("/live")
                .header("cookie", "iotkit_edge_session=valid; iotkit_edge_csrf=csrf")
    ...
    assert!(html.contains("有効な累積ルールがありません"));
    assert!(html.contains(r#"href="/sensors""#));
    assert!(html.contains("累積ルールを設定"));
