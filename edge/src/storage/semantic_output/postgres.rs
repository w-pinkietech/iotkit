async fn create_rule_postgres(
    ...
    let spec_json = serde_json::to_value(draft.spec)
    ...
    sqlx::query(
        "INSERT INTO semantic_rules(rule_id,signal_ref,display_name,kind,series_id,\
         revision,spec_json,active,created_at) VALUES($1,$2,$3,$4,$5,1,$6,TRUE,$7)",
    )
    .bind(&rule_id)
    .bind(&signal_ref)
    ...
    sqlx::query(
        "INSERT INTO semantic_rule_starts(rule_id,revision,ledger_epoch,start_after_pub_seq) \
         SELECT $1,1,ledger_epoch,accepted_through FROM accepted_cursors WHERE edge_node_id=$2",
    )
    .bind(&rule_id)
    .bind(&draft.edge_node_id)
    .execute(&mut **tx)
    .await?;
    sqlx::query("INSERT INTO semantic_rule_runtime(rule_id) VALUES($1)")
        .bind(&rule_id)
        .execute(&mut **tx)
        .await?;
...
async fn ensure_signal_postgres(
    ...
    if let Some(value) = sqlx::query_scalar(
        "SELECT signal_ref FROM semantic_signals WHERE edge_node_id=$1 AND series_key=$2",
    )
    .bind(edge_node_id)
    .bind(series_key)
    ...
    sqlx::query(
        "INSERT INTO semantic_calibration_starts(\
         ...
         SELECT $1,1,ledger_epoch,0 FROM accepted_cursors WHERE edge_node_id=$2",
    )
    .bind(&signal_ref)
    .bind(edge_node_id)
    .execute(&mut **tx)
...
async fn revise_rule_postgres(
    ...
    lock_edge_cursors_postgres(tx, &edge_node_id).await?;
    lock_rule_runtime_postgres(tx, rule_id).await?;
    let row = sqlx::query(
        "SELECT rule.signal_ref,signal.edge_node_id,signal.series_key,rule.kind,\
         ...
         JOIN semantic_signals AS signal ON signal.signal_ref=rule.signal_ref \
         WHERE rule.rule_id=$1 AND rule.active=TRUE FOR UPDATE",
    )
    .bind(rule_id)
    .fetch_optional(&mut **tx)
    .await?
    ...
    let old_spec: Value = row.try_get("spec_json")?;
    ...
    sqlx::query(
        "UPDATE semantic_rules SET display_name=$1,series_id=$2,revision=$3,spec_json=$4 \
         WHERE rule_id=$5",
    )
    .bind(display_name)
    .bind(&series_id)
    ...
    sqlx::query(
        "INSERT INTO semantic_rule_starts(rule_id,revision,ledger_epoch,start_after_pub_seq) \
         SELECT $1,$2,ledger_epoch,accepted_through FROM accepted_cursors WHERE edge_node_id=$3",
    )
    .bind(rule_id)
    .bind(revision)
    .bind(&edge_node_id)
...
async fn apply_reset_postgres(
    ...
    let kind: Option<String> = sqlx::query_scalar(
        "SELECT kind FROM semantic_rules WHERE rule_id=$1 AND active=TRUE FOR UPDATE",
    )
    .bind(rule_id)
    .fetch_optional(&mut **tx)
    .await?;
    ...
    sqlx::query(
        "INSERT INTO semantic_counter_resets(reset_id,rule_id,requested_at) VALUES($1,$2,$3)",
    )
    .bind(reset_id)
    .bind(rule_id)
    .bind(now)
    ...
    sqlx::query(
        "INSERT INTO semantic_counter_reset_boundaries(\
         ...
         ON cursor.edge_node_id=signal.edge_node_id WHERE rule.rule_id=$2",
    )
    .bind(reset_id)
    .bind(rule_id)
    .execute(&mut **tx)
...
async fn activate_profile_postgres(
    tx: &mut Transaction<'_, Postgres>,
    display_name: &str,
    ...
    .bind(Value::Object(values.clone()))
    .bind(initial)
    .bind(now)
    .execute(&mut **tx)
    .await?;
    let rows = sqlx::query(
        "SELECT rule.rule_id,rule.signal_ref,signal.edge_node_id,rule.kind \
         FROM semantic_rules AS rule JOIN semantic_signals AS signal \
         ON signal.signal_ref=rule.signal_ref WHERE rule.active=TRUE \
...
async fn create_binding_postgres(
    tx: &mut Transaction<'_, Postgres>,
    profile_id: &str,
    registration: &'static OutputAdapterRegistration,
    edge_id: &str,
    ...
    if modes.is_empty() {
        sqlx::query(
            "INSERT INTO output_bindings(binding_id,profile_id,rule_id,state,\
             ineligible_reason,revision,created_at) VALUES($1,$2,$3,'ineligible',$4,1,$5)",
        )
        .bind(&binding_id)
        .bind(profile_id)
        .bind(&rule.rule_id)
    ...
    if modes.len() > 1 {
        sqlx::query(
            "INSERT INTO output_bindings(binding_id,profile_id,rule_id,state,revision,created_at) \
             VALUES($1,$2,$3,'needs_configuration',1,$4)",
        )
        .bind(&binding_id)
        .bind(profile_id)
        .bind(&rule.rule_id)
    ...
    create_configured_binding_postgres(
        tx,
        profile_id,
        &binding_id,
        registration,
...
async fn identity_postgres(
    ...
    if let Some(row) = sqlx::query(
        "SELECT output_identity_id,external_id,confirmed_at IS NOT NULL AS confirmed \
         FROM output_identities WHERE adapter_id=$1 AND scope_key=$2 FOR UPDATE",
    )
    .bind(adapter_id)
    .bind(scope_key)
    .fetch_optional(&mut **tx)
    ...
    sqlx::query(
        "INSERT INTO output_identities(output_identity_id,adapter_id,scope_key,\
         external_id,created_at) VALUES($1,$2,$3,$4,$5)",
    )
    .bind(&identity_id)
    .bind(adapter_id)
    .bind(scope_key)
...
async fn create_configured_binding_postgres(
    tx: &mut Transaction<'_, Postgres>,
    profile_id: &str,
    binding_id: &str,
    registration: &'static OutputAdapterRegistration,
    ...
    let (identity_id, external, confirmed) =
        identity_postgres(tx, descriptor.id, &scope_key, policy.prefix, now).await?;
    ...
    sqlx::query(
        "INSERT INTO output_bindings(binding_id,profile_id,rule_id,output_identity_id,\
         mode,state,revision,created_at,activated_at) VALUES($1,$2,$3,$4,$5,$6,1,$7,$8)",
    )
    .bind(binding_id)
    .bind(profile_id)
    .bind(&rule.rule_id)
    ...
    let config_value: Value = serde_json::from_str(proposal.config.get())
        .map_err(|error| StorageError::InvalidOutput(error.to_string()))?;
    sqlx::query(
    ...
    .await?;
    if active {
        insert_binding_starts_postgres(tx, binding_id, &rule.edge_node_id).await?;
    }
    Ok(OutputBinding {
...
async fn insert_binding_starts_postgres(
    tx: &mut Transaction<'_, Postgres>,
    binding_id: &str,
    edge_node_id: &str,
) -> Result<(), StorageError> {
    sqlx::query(
        "INSERT INTO output_binding_starts(binding_id,ledger_epoch,start_after_pub_seq) \
         SELECT $1,ledger_epoch,accepted_through FROM accepted_cursors WHERE edge_node_id=$2 \
         ON CONFLICT(binding_id,ledger_epoch) DO NOTHING",
    )
    .bind(binding_id)
    .bind(edge_node_id)
    .execute(&mut **tx)
...
async fn auto_bind_rule_postgres(
    tx: &mut Transaction<'_, Postgres>,
    rule: &SemanticRule,
    now: i64,
) -> Result<(), StorageError> {
    ...
    for profile in profiles {
        ...
        let setup: Value = profile.try_get("setup_json")?;
        let values = setup
            .as_object()
            .cloned()
...
async fn configure_binding_postgres(
    tx: &mut Transaction<'_, Postgres>,
    binding_id: &str,
    mode: &str,
    values: Map<String, Value>,
    ...
    let row = sqlx::query(
        "SELECT binding.profile_id,binding.rule_id,profile.adapter_id,profile.setup_json,\
         ...
         JOIN semantic_signals AS signal ON signal.signal_ref=rule.signal_ref \
         WHERE binding.binding_id=$1 AND binding.state='needs_configuration' FOR UPDATE",
    )
    .bind(binding_id)
    .fetch_optional(&mut **tx)
    .await?
    ...
    sqlx::query("DELETE FROM output_bindings WHERE binding_id=$1")
        .bind(binding_id)
        .execute(&mut **tx)
        .await?;
    create_configured_binding_postgres(
...
async fn confirm_binding_postgres(
    tx: &mut Transaction<'_, Postgres>,
    binding_id: &str,
    now: i64,
) -> Result<(), StorageError> {
    let row = sqlx::query(
        "SELECT profile_id,output_identity_id FROM output_bindings \
         WHERE binding_id=$1 AND state='prepared' FOR UPDATE",
    )
    .bind(binding_id)
    .fetch_optional(&mut **tx)
    .await?
    ...
    sqlx::query(
        "UPDATE output_identities SET confirmed_at=COALESCE(confirmed_at,$1) \
         WHERE output_identity_id=$2",
    )
    .bind(now)
    .bind(&identity_id)
    .execute(&mut **tx)
    ...
    let rows = sqlx::query(
        "SELECT binding.binding_id,signal.edge_node_id FROM output_bindings AS binding \
         JOIN semantic_rules AS rule ON rule.rule_id=binding.rule_id \
         JOIN semantic_signals AS signal ON signal.signal_ref=rule.signal_ref \
         WHERE binding.profile_id=$1 AND binding.output_identity_id=$2 \
         AND binding.state='prepared'",
    )
    .bind(&profile_id)
    .bind(&identity_id)
    .fetch_all(&mut **tx)
    .await?;
    for item in rows {
        insert_binding_starts_postgres(
            tx,
            &item.try_get::<String, _>("binding_id")?,
            &item.try_get::<String, _>("edge_node_id")?,
        )
        .await?;
    }
    sqlx::query(
        "UPDATE output_bindings SET state='active',revision=revision+1,activated_at=$1 \
         WHERE profile_id=$2 AND output_identity_id=$3 AND state='prepared'",
    )
    .bind(now)
    .bind(&profile_id)
    .bind(&identity_id)
    ...
    .await?;
    sqlx::query(
        "UPDATE output_routes SET active=TRUE,lifecycle_state='active' WHERE binding_id IN \
         (SELECT binding_id FROM output_bindings WHERE profile_id=$1 \
          AND output_identity_id=$2 AND state='active')",
    )
    .bind(&profile_id)
    .bind(&identity_id)
    .execute(&mut **tx)
    ...
    sqlx::query(
        "UPDATE export_profiles SET state='active',revision=revision+1 \
         WHERE profile_id=$1 AND state='preparing'",
    )
    .bind(&profile_id)
    .execute(&mut **tx)
    .await?;
...
async fn stop_profile_postgres(
    tx: &mut Transaction<'_, Postgres>,
    profile_id: &str,
    now: i64,
) -> Result<(), StorageError> {
    let result = sqlx::query(
        "UPDATE export_profiles SET state='draining',revision=revision+1 \
         WHERE profile_id=$1 AND state IN ('preparing','active')",
    )
    .bind(profile_id)
    .execute(&mut **tx)
    .await?;
    ...
    sqlx::query(
        "INSERT INTO output_binding_ends(binding_id,ledger_epoch,end_at_pub_seq) \
         ...
         WHERE binding.profile_id=$1 AND binding.state='active' \
         ON CONFLICT(binding_id,ledger_epoch) DO NOTHING",
    )
    .bind(profile_id)
    .execute(&mut **tx)
    ...
    sqlx::query(
        "UPDATE output_bindings SET state=CASE WHEN state='active' THEN 'draining' ELSE 'stopped' END,\
         revision=revision+1,stopped_at=CASE WHEN state='active' THEN NULL ELSE $1 END \
         WHERE profile_id=$2 AND state IN ('active','prepared','needs_configuration','ineligible')",
    )
    .bind(now)
    .bind(profile_id)
    .execute(&mut **tx)
    ...
    sqlx::query(
        "UPDATE output_routes SET lifecycle_state='draining' WHERE binding_id IN \
         (SELECT binding_id FROM output_bindings WHERE profile_id=$1 AND state='draining')",
    )
    .bind(profile_id)
    .execute(&mut **tx)
    .await?;
    reconcile_profiles_postgres(tx, now).await
}

async fn list_profiles_postgres(
    pool: &sqlx::PgPool,
    ...
    for profile in profiles {
        ...
        let rows = sqlx::query(
            "SELECT binding.binding_id,binding.rule_id,COALESCE(identity.external_id,'') external_id,\
             ...
             WHERE binding.profile_id=$1 ORDER BY binding.created_at,binding.binding_id",
        )
        .bind(&profile_id)
        .fetch_all(pool)
        .await?;
...
async fn claim_postgres(
    ...
    let row = sqlx::query(
        ...
        WHERE ranked.route_rank=1
          AND (outbox.claim_token IS NULL OR outbox.claim_until<=$1)
    ...
    let result = sqlx::query(
        "UPDATE output_outbox SET claim_token=$1,claimed_at=$2,claim_until=$3,\
         attempts=attempts+1 WHERE export_id=$4 AND published_at IS NULL \
         AND (claim_token IS NULL OR claim_until<=$2)",
    )
    .bind(token)
    .bind(now)
...
async fn project_postgres(
    tx: &mut Transaction<'_, Postgres>,
    adapters: &'static [OutputAdapterRegistration],
) -> Result<Option<ProjectedOne>, StorageError> {
    let retry = retry_route_postgres(tx, adapters).await?;
    if let Some(reset) = ready_reset_postgres(tx).await? {
        let publications =
            retry.publications + enqueue_routes_postgres(tx, &reset, adapters).await?;
    ...
    let Some(rule_id) = ready_rule_postgres(tx).await? else {
    ...
    let Some(candidate) = candidate_postgres(tx, &rule_id).await? else {
        return Ok(retry.attempted.then_some(ProjectedOne {
            receipt: false,
            observation: false,
            publications: retry.publications,
    ...
    let measurement = match decode_measurement(&candidate) {
        ...
        Err(StorageError::InvalidSemantic(_)) => {
            return record_projection_failure_postgres(tx, &candidate, retry.publications).await;
        }
    ...
    let runtime = sqlx::query(
        "SELECT initialized,detector_active,counter,pending,pending_active,pending_since,\
         ...
         applied_series_id,next_sequence FROM semantic_rule_runtime WHERE rule_id=$1",
    )
    ...
    if let Some(observation) = &produced {
        insert_observation_postgres(tx, observation).await?;
        publications += enqueue_routes_postgres(tx, observation, adapters).await?;
        next_sequence += 1;
    }
    sqlx::query(
        "INSERT INTO semantic_projection_receipts(rule_id,ledger_epoch,pub_seq,revision,\
         calibration_revision,observation_id) VALUES($1,$2,$3,$4,$5,$6)",
    )
    .bind(&candidate.rule_id)
...
async fn record_projection_failure_postgres(
    tx: &mut Transaction<'_, Postgres>,
    ...
    sqlx::query(
        "INSERT INTO semantic_projection_failures(rule_id,ledger_epoch,pub_seq,error_code,\
         attempts,last_failed_at) VALUES($1,$2,$3,'invalid_observation',1,$4) \
         ON CONFLICT(rule_id,ledger_epoch,pub_seq) DO UPDATE SET \
         attempts=semantic_projection_failures.attempts+1,last_failed_at=EXCLUDED.last_failed_at",
    )
    .bind(&candidate.rule_id)
    .bind(&candidate.ledger_epoch)
    .bind(candidate.pub_seq)
    ...
    sqlx::query(
        "INSERT INTO semantic_projection_receipts(rule_id,ledger_epoch,pub_seq,revision,\
         calibration_revision,observation_id) VALUES($1,$2,$3,$4,$5,NULL)",
    )
    .bind(&candidate.rule_id)
    .bind(&candidate.ledger_epoch)
    .bind(candidate.pub_seq)
...
async fn candidate_postgres(
    ...
    row.map(|row| {
        ...
        Ok(ProjectionCandidate {
            ...
            current_spec: serde_json::from_value(spec)
                .map_err(|error| StorageError::InvalidSemantic(error.to_string()))?,
...
async fn insert_observation_postgres(
    ...
    let value_json: Value = serde_json::from_slice(&value.value_json)
        .map_err(|error| StorageError::InvalidSemantic(error.to_string()))?;
    sqlx::query(
...
async fn retry_route_postgres(
    ...
    let row = sqlx::query(
        "SELECT observation.observation_id,observation.rule_id,observation.revision,\
         ...
         observation.kind,observation.value_json::text AS value_json,observation.reading,\
         observation.signal_ref,observation.edge_node_id,observation.ledger_epoch,\
         ...
         COALESCE((SELECT MAX(last_attempt_at) FROM output_route_attempts attempt \
           WHERE attempt.route_id=route.route_id),0),\
         route.created_at,observation.observation_row_id LIMIT 1 FOR UPDATE OF route SKIP LOCKED",
    )
    .fetch_optional(&mut **tx)
    .await?;
    let Some(row) = row else {
    ...
    let observation = ProducedObservation {
        ...
        kind,
        value_json: value_json.into_bytes(),
        value: decoded.value,
...
async fn enqueue_routes_postgres(
    tx: &mut Transaction<'_, Postgres>,
    ...
    let routes = sqlx::query(
        "SELECT route.route_id,route.adapter_id,route.config_schema_version,route.config_json \
         ...
         AND NOT EXISTS(SELECT 1 FROM output_route_attempts attempt \
           WHERE attempt.route_id=route.route_id AND attempt.observation_id<>$2) \
         AND (NOT EXISTS(SELECT 1 FROM output_binding_starts start \
           WHERE start.binding_id=binding.binding_id AND start.ledger_epoch=$3) \
           OR EXISTS(SELECT 1 FROM output_binding_starts start \
             WHERE start.binding_id=binding.binding_id AND start.ledger_epoch=$3 \
             AND $4>start.start_after_pub_seq)) \
         AND (NOT EXISTS(SELECT 1 FROM output_binding_ends finish \
           WHERE finish.binding_id=binding.binding_id AND finish.ledger_epoch=$3) \
           OR EXISTS(SELECT 1 FROM output_binding_ends finish \
             WHERE finish.binding_id=binding.binding_id AND finish.ledger_epoch=$3 \
             AND $4<=finish.end_at_pub_seq)) ORDER BY route.created_at,route.route_id",
    )
    ...
    for route in routes {
        ...
        let config: Value = route.try_get("config_json")?;
        let config = serde_json::value::to_raw_value(&config)
            .map_err(|error| StorageError::InvalidOutput(error.to_string()))?;
        match registration.adapter.transform(&config, &adapter_observation) {
            Ok(publication) => {
                let result = sqlx::query(
                    "INSERT INTO output_outbox(export_id,route_id,observation_id,\
                     topic,qos,retain,payload_json,created_at) VALUES($1,$2,$3,$4,$5,$6,$7,$8) \
                ...
                sqlx::query(
                    "DELETE FROM output_route_attempts WHERE route_id=$1 AND observation_id=$2",
                )
                .bind(&route_id)
                .bind(&observation.observation_id)
                .execute(&mut **tx)
                ...
                sqlx::query(
                    "UPDATE output_routes SET last_transform_error_code=NULL,\
                     last_transform_error_at=NULL,last_transform_success_at=$1 WHERE route_id=$2",
                )
                .bind(observation.created_at)
                .bind(&route_id)
                .execute(&mut **tx)
...
async fn record_route_error_postgres(
    tx: &mut Transaction<'_, Postgres>,
    route_id: &str,
    observation_id: &str,
    code: &str,
    ...
    sqlx::query(
        "UPDATE output_routes SET last_transform_error_code=$1,last_transform_error_at=$2 \
         WHERE route_id=$3",
    )
    .bind(code)
    .bind(now)
    ...
    sqlx::query(
        "INSERT INTO output_route_attempts(route_id,observation_id,attempts,last_attempt_at,error_code) \
         VALUES($1,$2,1,$3,$4) ON CONFLICT(route_id,observation_id) DO UPDATE SET \
         attempts=output_route_attempts.attempts+1,last_attempt_at=EXCLUDED.last_attempt_at,\
...
async fn update_runtime_postgres(
    tx: &mut Transaction<'_, Postgres>,
    candidate: &ProjectionCandidate,
    ...
    sqlx::query(
        "UPDATE semantic_rule_runtime SET initialized=$1,detector_active=$2,counter=$3,pending=$4,\
         pending_active=$5,pending_since=$6,applied_revision=$7,\
         applied_calibration_revision=$8,applied_ledger_epoch=$9,applied_series_id=$10,\
         next_sequence=$11 WHERE rule_id=$12",
    )
...
async fn ready_reset_postgres(
    ...
    insert_observation_postgres(tx, &produced).await?;
    sqlx::query(
        "UPDATE semantic_rule_runtime SET initialized=FALSE,detector_active=FALSE,counter=0,\
         pending=FALSE,pending_active=FALSE,pending_since=0,applied_revision=$1,\
         applied_calibration_revision=$2,applied_ledger_epoch=$3,applied_series_id=$4,\
         next_sequence=$5 WHERE rule_id=$6",
    )
    ...
    .await?;
    sqlx::query(
        "UPDATE semantic_counter_resets SET applied_at=$1,zero_observation_id=$2 \
         WHERE reset_id=$3 AND applied_at IS NULL",
    )
...
async fn reconcile_profiles_postgres(
    tx: &mut Transaction<'_, Postgres>,
    now: i64,
) -> Result<(), StorageError> {
    sqlx::query(
        "UPDATE output_bindings SET state='stopped',revision=revision+1,stopped_at=$1 \
    ...
    .await?;
    sqlx::query(
        "UPDATE output_routes SET active=FALSE,lifecycle_state='stopped' WHERE binding_id IN \
         (SELECT binding_id FROM output_bindings WHERE state='stopped')",
    )
    .execute(&mut **tx)
    .await?;
    sqlx::query(
        "UPDATE export_profiles SET state='stopped',stopped_at=$1,revision=revision+1 \
...
async fn lock_rule_runtime_postgres(
    tx: &mut Transaction<'_, Postgres>,
    rule_id: &str,
) -> Result<(), StorageError> {
    sqlx::query("SELECT rule_id FROM semantic_rule_runtime WHERE rule_id=$1 FOR UPDATE")
        .bind(rule_id)
        .fetch_one(&mut **tx)
        .await?;
    Ok(())
...
async fn lock_signal_runtimes_postgres(
    tx: &mut Transaction<'_, Postgres>,
    signal_ref: &str,
) -> Result<(), StorageError> {
    sqlx::query(
        "SELECT runtime.rule_id FROM semantic_rule_runtime AS runtime \
         JOIN semantic_rules AS rule ON rule.rule_id=runtime.rule_id \
         WHERE rule.signal_ref=$1 ORDER BY runtime.rule_id FOR UPDATE OF runtime",
    )
    .bind(signal_ref)
    .fetch_all(&mut **tx)
    .await?;
