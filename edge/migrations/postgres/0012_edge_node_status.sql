...
CREATE TABLE edge_node_status (
    ...
    pending_since_at BIGINT CHECK(pending_since_at IS NULL OR pending_since_at >= 0)
);
...
-- The digest narrows this current-epoch lookup; application SQL rechecks the
-- full series key to retain exact identity.
CREATE INDEX ix_raw_records_diagnostic_epoch_signal_received
ON raw_records(edge_node_id, ledger_epoch, md5(series_key), received_at DESC, pub_seq DESC);
