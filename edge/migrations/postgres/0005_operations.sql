...
CREATE TABLE edge_restore_events (
    ...
    restored_at BIGINT NOT NULL CHECK(restored_at >= 0),
    backup_created_at BIGINT NOT NULL CHECK(backup_created_at >= 0),
    backup_edge_id TEXT NOT NULL,
    backup_schema_version BIGINT NOT NULL CHECK(backup_schema_version > 0),
...
CREATE TABLE edge_restore_cursor_checks (
    ...
    edge_node_id TEXT NOT NULL,
    ledger_epoch TEXT NOT NULL,
    backup_accepted_through BIGINT NOT NULL CHECK(backup_accepted_through >= 0),
    state TEXT NOT NULL CHECK(state IN ('pending', 'matched', 'recovery_required', 'archive_lost')),
    observed_cursor_start BIGINT,
