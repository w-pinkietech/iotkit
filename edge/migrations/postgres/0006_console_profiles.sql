...
SELECT 'dev_' || replace(gen_random_uuid()::text,'-',''),edge_node_id,system_id,updated_at
FROM descriptor_devices;

INSERT INTO inventory_signals(signal_ref,edge_node_id,series_key,system_id,created_at)
SELECT 'sig_' || replace(gen_random_uuid()::text,'-',''),edge_node_id,series_key,system_id,updated_at
...
CREATE TABLE device_profiles (
    ...
    location TEXT NOT NULL CHECK(char_length(location) BETWEEN 1 AND 256),
...
CREATE TABLE signal_profiles (
    ...
    display_unit TEXT NOT NULL CHECK(char_length(display_unit) <= 32),
