...
pub fn run_status(
    conn: &Connection,
    identity: &iotkit_core_ledger::EdgeNodeIdentity,
    args: StatusArgs,
) -> AppResult<()> {
    if args.pub_seq < 1 {
    ...
    if args.ledger_epoch != identity.ledger_epoch {
        return Err(
            "cannot determine smoke delivery after the Edge Node ledger epoch changed".into(),
        );
    }
    let target = target_get(conn)?.ok_or("MQTT IoTKit Edge target is not initialized")?;
    if target.target_id != "edge"
        || !target.archive_responsible
        || !target.credential_token.is_empty()
    {
        return Err("configured exit target is not the MQTT IoTKit Edge target".into());
    }
