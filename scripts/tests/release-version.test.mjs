...
test("requires non-empty evidence and rejects malformed nested entries", () => {
  withFixtureRepository((root) => {
    ...
    manifest.domains[2].types = "types/reference.rs";
    manifest.domains[3].schemas = [];
    manifest.domains[4].tests = [];
    manifest.storage[0].authority = [];
    manifest.storage[1].schema = [];
    manifest.storage[2].tests = [];
    ...
    assert.ok(problems.some((problem) => problem.includes("domains[2].types must be an array")));
    assert.ok(problems.some((problem) => problem.includes("domains[3].schemas must not be empty")));
    assert.ok(problems.some((problem) => problem.includes("domains[4].tests must not be empty")));
    assert.ok(problems.some((problem) => problem.includes("storage[0].authority must not be empty")));
    assert.ok(problems.some((problem) => problem.includes("storage[1].schema must not be empty")));
    assert.ok(problems.some((problem) => problem.includes("storage[2].tests must not be empty")));
    assert.ok(problems.some((problem) => problem.includes("storage[3] must be an object")));
