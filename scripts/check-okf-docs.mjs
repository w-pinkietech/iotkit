...
const bundleRoot = path.join(repoRoot, "docs", "okf");
const locales = ["ja", "en"];
const required = ["type", "title", "description", "language", "translation_key", "status", "revision"];
const allowedTypes = new Set(["Concept", "Architecture", "Contract Overview", "Runbook Overview"]);
...
if (!fs.existsSync(bundleRoot)) {
  console.error("docs/okf does not exist");
  process.exit(1);
}

const rootIndex = path.join(bundleRoot, "index.md");
const root = parseFrontmatter(rootIndex, fs.readFileSync(rootIndex, "utf8"));
if (!root || root.metadata.okf_version !== "0.1") fail(rootIndex, 'bundle root must declare okf_version: "0.1"');
...
for (const file of bundleFiles(bundleRoot)) {
  ...
  if (basename === "log.md") {
    fail(file, "log.md is not supported by the current IoTKit OKF profile");
...
if (baseRef) {
  ...
  try {
    const changes = execFileSync("git", ["diff", "--name-status", "-M", `${baseRef}...HEAD`, "--", "docs/okf"], {
      cwd: repoRoot,
      encoding: "utf8",
    })
      .trim()
    ...
    for (const line of changes) {
      ...
      if (status.startsWith("R") || status.startsWith("C")) {
        changed.add(second);
        previousPath.set(second, first);
      } else {
        changed.add(first);
        if (status === "M") previousPath.set(first, first);
      }
    }
    const baseFiles = execFileSync("git", ["ls-tree", "-r", "--name-only", baseRef, "--", "docs/okf"], {
      cwd: repoRoot,
      encoding: "utf8",
    })
      .trim()
      .split(/\r?\n/)
      .filter((file) => file.endsWith(".md") && !file.endsWith("/index.md") && !file.endsWith("/log.md"));
  ...
  for (const translationKey of translationKeys) {
    ...
    if (!changed.has(jaPath) && !changed.has(enPath)) continue;
    if (!changed.has(jaPath) || !changed.has(enPath)) {
...
console.log(`IoTKit OKF profile validation passed: ${translationKeys.size} bilingual concepts.`);
