...
- **uptime**: the time elapsed from the start of the device process to the receipt of the input that settled the output. It comes from a monotonic clock, so within one process run the difference between two Observations equals the real elapsed time. It resets on restart without changing the series. Cycle times and gap lengths are measured with it.
