# redesign → product docs gap inventory

**Status:** working survey for [#141](https://github.com/w-pinkietech/iotkit/issues/141)
**As-of:** 2026-08-01
**Authority:** none. This file is not product documentation. `docs/product/` is the
...
- **`docs/product/`** is the only human-readable product authority (OKF v0.2 packaging).
- **`docs/superpowers/` stays.** Clear lineage as sprint design/plans; do not
  ...
  are not current law. Specs→product docs absorbs are secondary ([#143](https://github.com/w-pinkietech/iotkit/issues/143)).
- **`docs/redesign/` is the main risk and the main absorb path.** It is easy to
  misread as a second current corpus. Fix by rewriting still-true gaps into product docs
...
Classify `docs/redesign/` material against current product docs so later work can
**rewrite only still-true, still-normative gaps into product docs**—not copy redesign
...
- **absorb-product** — write fresh product-docs prose (both languages, revision++), verify
...
|---|---|---|
| Already in product docs (core) | D1, D7–D10, and parts of D2/D4/D5/D11 | Custody, ingest, adapters, product model, architecture map |
| Gap candidates for product-docs rewrite | Few focused topics | See “Priority absorb list” |
...
Product contracts and architecture. What remains is not “move the tree,” but a
**small set of concepts** still thin in product docs, plus **lots of dated evidence** that
...
Ordered by (value × confidence that content is still true × product-docs thinness).
Each becomes its **own later issue/PR** after human pick.

| ID | Topic | Suggested product-doc type | Why gap | Still true? (provisional) |
|---|---|---|---|---|
...
| Artifact | Still true? | In product docs? | Normative now? | Action | Notes |
|---|---|---|---|---|---|
...
| **D2** authority, commissioning, recovery image | **Partial** | **Partial** — product-model, architecture, custody, recovery, install | Yes where product docs/ops say so | **absorb-product** only if specific missing commissioning invariants found; else **skip** | Many ops details live in install/recovery runbooks |
...
| **D4** adapter anatomy | **Partial** | **Partial** — input-adapter-v1, crate map | Northbound yes | **absorb-product** candidate **G3** | Care-servicer half points at D12 |
| **D5** series identity | **Yes** (system_id / hardware_id / user_label) | **Partial** — architecture bullet; not a Concept | Yes in implementation | **absorb-product** candidate **G1** | Strong “why” still mostly in D5 |
| **D6** measurement registry | **Partial** | **Partial** — registry crate mentioned | If code matches two-layer | **absorb-product** candidate **G2** after code check | `edge-node/core/registry/src/lib.rs` still calls redesign D6 canonical; replace that pointer in a later scoped cleanup. |
...
| **D11** ingress auth / admission | **Partial** | **Partial** — ingest-v1 | Only shipped HTTP path | **absorb-product** only after code check **G6**; else **leave-evidence** | Pairing windows / flow classes may exceed v1 |
...
| **D13** UI scope | **Yes** (thin UI, no secret redisplay or Broker/TLS credential management; Console ≠ broker provision) | **Partial** — product-model, install | Yes for Console | **absorb-product** candidate **G5** | |
| **terminology.md** (redesign) | **Partial** (4-tier + bans useful; “Edge is Go” false) | **Yes** thinner product terminology | product terms yes | **skip** product docs as current; redesign = evidence of older fuller glossary | Optional later: expand product terminology from *verified* rows only |
| **responsibility-ledger.md** R1–R23 | **Partial** (mix of done / future) | **No** as R-index | **No** as full list | **absorb-product** candidate **G4** as *implemented* map only | Do not paste R1–R23 wholesale into product docs |
...
| Artifact | Still true? | In product docs? | Normative now? | Action | Stale examples |
|---|---|---|---|---|---|
...
| **diagrams/*.html** | Unverified visuals | No | **No** | **leave-evidence** | Not synced to product docs |

**Do not rewrite these to “match product docs.”** Do not absorb into product docs.
...
| Component names | gateway, host-agent, iotkit-site era | Use product-doc terms only |
| Authority meta | “redesign is current authority” | False; product docs are current |
...
2. Per item: code skim → draft product docs ja+en → `node scripts/check-product-docs.mjs` → PR closing a child issue.
...
```text
1. Current behavior / rules  → docs/product/ + schema/types + fixtures + conformance tests
