...
| IoTKit Edgeの保存運用 | IoTKit Edge DB使用量、hostの利用可能容量、raw・意味付け・outboxの件数と滞留を確認できる。rawの保持期間と削除権威を明示し、未配送outboxや復旧に必要な状態を容量対策として黙って削除しない。v1で実装する`embedded` profileについて、対象hardware、Edge Node/sensor数、records/秒、保持量、同時query、backup時間を含む検証済みcapacity envelopeと、現在値・増加速度・残り日数を示す | 容量watermark test、retention境界test、Console/API E2E、容量不足negative test、production-shaped負荷/backup/復旧test |
