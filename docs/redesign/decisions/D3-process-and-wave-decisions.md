...
> IoTKitは、さまざまなセンサーを小さなadapterで簡単につなぎ、IoTKit Edge Nodeで現場データを
> 黙って失うことなく収集・正規化・保全・再送し、IoTKit Edgeへ届けるIoTデータ収集基盤である。

この価値は次の2点の組み合わせにある。

1. **adapterの作りやすさ**: adapter作者はセンサー固有の通信、設定、値の読み取り、measurementへの
   写像だけを担当する。SQLite、MQTT、再送、保管責任、retention、認証をadapterへ持ち込まない。
2. **保管責任の確実な引き渡し**: Edge Nodeは観測を耐久保存し、IoTKit Edgeがraw recordを耐久保存したと確認できる
   まで保持する。MQTT PUBACKだけではEdge Nodeの保管責任を解除しない。
...
> IoTKit Consoleと手順書から状態確認と復旧ができる。

対象topologyは、1つのIoTKit Edgeに1台以上のIoTKit Edge Nodeとする。同一導入内の複数Edge NodeはIoTKit Edgeが扱う。
...
| 領域 | 完了条件 | 必要な証拠 |
|---|---|---|
| 新規導入 | 既存DB、設定、credentialを流用せず、新しいLinux IoTKit Edge hostとRaspberry PiへEdge Node、Broker、IoTKit Edgeを導入できる。最初の`system_admin`をIoTKit Edge hostのlocal CLIで作成し、descriptorで発見したEdge NodeをIoTKit Consoleでactivationした後、commissioning smokeをIoTKit Edge受理まで通せる。完全無人導入と、IoTKitによるLAN、DNS、router、firewall構築は要求しない | 新規一時環境の導入test、公開CLIの出力、1ページから辿れる導入・診断手順 |
| 実センサーとadapter境界 | BravePI MainboardのUARTから温度と接点入力を取得し、Edge Nodeで耐久保存してIoTKit Edgeへ渡せる。BravePI固有処理はadapter内へ閉じ、疑似adapterまたは別adapterが同じingest契約を使える | Raspberry PiとBravePIによる温度・接点の実信号証拠、host上のadapter境界test |
| 複数Edge Node | 1つのIoTKit Edgeへ複数Edge Nodeを同時接続でき、identity、credential、topic、raw record、cursor、表示が混ざらない | BravePI実機Edge Node 1台の試験に加え、host上で疑似Edge Node 2台を同時接続する統合test |
| IoTKit Console | 工場LANのWindows browserからCaddy HTTPS経由で利用できる。全画面でloginを要求し、個人accountと`viewer`、`admin`、`system_admin`を分離する。Edge Node、device、signalを別階層で表示し、descriptorで発見したEdge Nodeの初回Edge Node activation、sensor type、現在値、最終受信時刻、停止・古い・未設定状態を日本語で確認できる。表示名と設置場所を設定でき、変更者と変更内容を監査できる | browser E2E、activation権限・再送・未登録recordsのnegative test、監査と秘密非表示test、現場担当者役による操作確認 |
| 履歴と汎用export | sensor、Edge Node、期間を指定してIoTKit Edgeに保存された履歴を検索できる。大量データは有界queryと時間集約でgraph表示する。標準CSVは保存済みの補正・判定・累積結果を出し、受信した生データCSVは調査用として分離する。画面内の最新数件だけを履歴機能とは扱わない | query境界・pagination・集約test、加工後/生データCSV escaping test、Console browser E2E、長期間・高頻度データの負荷test |
| IoTKit Edgeでの意味付け | IoTKitの汎用語彙として、数値の現在値、booleanの現在状態、接点・光等から作る累積値、補正、しきい値による状態・alarmを扱える。設定はfuture-onlyで、過去値を暗黙に再計算しない。保存前に実信号previewで挙動を確認できる | evaluatorの境界test、Console/API E2E、mapping revisionとfuture-only test |
| 外部application出力 | IoTKit Edgeの意味付けと、出力先固有のpayload変換を分離し、Output Adapterを追加できる。YokaKit Adapterは合意済みの`source-id`、`signal-id`、observation contractを使い、IoTKitの累積値を`kind=production`へ、boolean状態を`onoff`または`gantt_chart`へ、alarm判定を`alarm`へ変換し、source statusも送れる。barcodeはIoTKit v1で直接取り込まない | YokaKit契約fixture、YokaKitとのconsumer contract test、YokaKit非依存のOutput Adapter境界test |
| 配送と保管責任 | Edge Node activation後にpublication admissionされたrecordはIoTKit Edgeの`accepted-through`までEdge Nodeが保持する。登録前ローカル値はpublication identityを持たずIoTKit Edgeへ送らない。IoTKit Edgeが受理したraw recordは再起動後も残る。外部出力eventはMQTT PUBACKまでIoTKit Edge outboxへ残り、at-least-once再送時に同じeventを識別できる | Edge Node/IoTKit Edge/Broker再起動、activation境界、未登録records拒否、経路断、重複、DB書込失敗、outbox再送・収束test |
...
| IoTKit Edgeの認証と復旧 | password、credential、秘密鍵をlog、監査、画面、Gitへ出さない。初期所有権と緊急password復旧はIoTKit Edge hostのlocal CLIだけで行い、sessionを失効できる。IoTKit ConsoleはHTTPS失敗時にHTTPへfallbackしない | account/session統合test、secret scan、local CLI復旧test、Caddy障害test |
| IoTKit Edgeの保存運用 | IoTKit Edge DB使用量、hostの利用可能容量、raw・意味付け・outboxの件数と滞留を確認できる。rawの保持期間と削除権威を明示し、未配送outboxや復旧に必要な状態を容量対策として黙って削除しない。`embedded`と`postgres`の各profileについて、対象hardware、Edge Node/sensor数、records/秒、保持量、同時query、backup時間を含む検証済みcapacity reportと、現在値・増加速度・取得可能な場合の残り日数を示す。reportのない規模は対応済みと表現しない | 容量watermark test、retention境界test、Console/API E2E、容量不足negative test、profile別負荷/backup/復旧test |
| backup・復元・更新 | IoTKit Edgeの整合したbackupを秘密情報を漏らさず作成し、新規環境へ検証付きで復元できる。復元中の書込みを防ぎ、失敗時は元DBを保持する。手動version更新は事前backup、migration、health確認、既知良好versionへのrollback手順を持つ | backup/restore往復test、破損・異version・容量不足negative test、更新/rollback rehearsalと手順書 |
| 障害診断 | Consoleとlocal診断から、sensor停止、Input Adapter停止、Edge Node停止、Edge Node-Broker通信断、IoTKit Edge受信停止、外部Broker配送停止、outbox滞留、容量警告を区別できる。推測で「稼働中」と表示しない | 状態組合せtest、stale境界test、障害注入E2E、診断手順 |

「データを黙って失わない」の検証境界は次とする。

1. Edge Node activation後にpublication admissionされたrecordは、IoTKit Edgeが同じrecordを耐久保存して有効な
   `accepted-through`を返すまでEdge Nodeが保持する。activation前のローカル確認値はpublication identityを
   持たず、IoTKit Edge custodyや後日replayの対象にしない。
2. Edge Nodeで容量不足等により耐久保存できなかった入力は成功応答せず、operatorが診断できる状態を残す。
3. IoTKit Edgeが受理したraw recordとcursorは同じtransactionで確定し、commit失敗時は
   `accepted-through`を返さない。
4. IoTKit Edgeのapplication eventは、出力先BrokerのPUBACKまでoutboxへ残す。application固有の業務処理成功を
   IoTKitの配送成功とは偽らない。
...
BravePI温度・接点
  -> IoTKit Edge Node
  -> MQTT Broker
  -> IoTKit ConsoleでEdge Nodeを発見・activation
  -> IoTKit Edge raw保存
  -> IoTKit Consoleで現在値と状態を確認
  -> 期間を指定して履歴graphを確認し、同じ条件をCSV出力
  -> 累積値・boolean状態・alarmを設定
  -> YokaKit Adapterから合意済みMQTT契約で出力
  -> Edge Node/IoTKit Edge/Broker再起動と通信断
  -> 復旧後にraw cursorと未配送outboxが収束
```

この実機Edge Node 1台のシナリオに、host上の疑似Edge Node 2台同時接続、IoTKit Edge DB書込失敗、MQTT認証失敗、
誤CA・誤hostname・期限切れcertificate、sensor信号停止、password紛失時のlocal CLI復旧を加える。
さらにIoTKit Edge容量watermark、整合backupからの復元、手動更新の失敗とrollbackをhost上で確認する。
...
- IoTKit ConsoleからのBroker endpoint、certificate、credential、ACLの作成・編集・切替
...
Temperature sensor
  ...
  -> BravePI Mainboard
  -> UART
  -> IoTKit Edge Node on Raspberry Pi
  -> MQTT QoS 1
  -> MQTT Broker
  -> IoTKit Edge
  -> raw SQLite
...
- Edge Nodeでのcanonical record化、readingとoutboxのatomic保存
- Edge NodeのMQTT publish、`accepted-through`検証、cursor更新、保管責任に基づくretention
- Mosquitto等のMQTT Brokerと、Edge Nodeごとのstatic credential/topic ACL
- IoTKit Edgeでのraw recordと連続cursorのatomic保存、commit後の`accepted-through` publish
- 重複再送の冪等処理と、同一identity・異内容のconflict検出
- IoTKit Edge raw dataの直接CLI query
- Dockerによる開発・障害試験環境と、Raspberry Pi上のsystemd実行

### 完了条件

- ペアリング済みBravePI Transmitter 1台の温度観測が、UART経由でEdge NodeとIoTKit Edgeの双方に記録される。
- 実frameからsensor typeとpayloadを確認し、仮定と異なる場合も正しいdriverへ明示的に対応付ける。
- `device_number`、RSSI、battery、温度値が期待どおりdecode・保存される。
- Edge NodeまたはBravePI Mainboardの再起動後、UART受信が自動的に復帰する。
- 保持容量内では、IoTKit Edge、Broker、ネットワークの停止中もEdge Nodeが収集を継続し、復旧後に欠けなく再送する。
- 同じbatchの再送でIoTKit Edgeのraw rowが重複せず、異内容ならconflictとしてcursorを進めない。
- IoTKit EdgeのSQLite commit失敗時は`accepted-through`を返さない。
- MQTT PUBACKだけではEdge Node cursorやpurge eligibilityが進まず、検証済み`accepted-through`だけで進む。
- Edge Node、Broker、IoTKit Edgeを個別に再起動しても同じ状態へ収束する。
- Docker試験に加え、Raspberry Piと実センサーで一連の流れを再現できる。

### 実機検証状況 (2026-07-13)

Raspberry Pi (Debian 13 / arm64) とペアリング済みBravePI実機で、現在のEdge Nodeから
IoTKit Edgeまでの縦経路を確認した。
...
- 温度センサーはEdge Node上で `hardware_id = ble:246880020140018b`、
...
- 停止後のSQLiteに`PRAGMA quick_check`を実行し`ok`を確認した。試験終了後はEdge Nodeが
  停止し、UARTが解放されることも確認した。
- 同じPi上の独立したDocker環境で、認証・topic ACL付きMosquittoとIoTKit Edgeを起動した。
  Edge Nodeに保持されていた`pub_seq` 1〜8をMQTT QoS 1で送り、IoTKit Edge raw SQLiteの8行と、
  IoTKit Edge commit後の`accepted-through = 8`によるEdge Node cursor更新を確認した。
- UART収集とMQTT出口を同時に35秒動かし、新しい温度3観測が`pub_seq` 9〜11としてIoTKit Edgeへ
  保存され、Edge Node cursorも11へ進んだ。IoTKit Edge CLIからraw recordを直接照会できた。
- broker再起動時、IoTKit Edgeが再接続後にrecords topicを再購読しない不具合を実機で発見した。
  購読をPahoの接続callbackへ移し、broker再起動をend-to-end試験へ追加した。修正版IoTKit Edgeは
  containerを再起動せず再購読し、その後の実機配送と`accepted-through`を完了した。

2026-07-14にはEdge Node/IoTKit Edge命名変更後のcommit `c0826a0`を使い、実験用labを旧DB・旧設定・旧MQTT
...
新しい温度観測11件が`pub_seq` 1〜11としてIoTKit Edgeの11行へ同じ`edge_node_id`・`ledger_epoch`で
保存され、Edge Nodeの`accepted-through` cursorも11へ収束した。停止後のEdge Node DBとIoTKit Edge DBはともに
`PRAGMA quick_check = ok`であり、Edge Node停止後にUARTが解放されたことも確認した。破棄したpre-releaseの
identity key、topic namespace、credentialは再利用していない。

同日の最終レビュー修正後commit `b6b9402`でも、上記の新形式DBを再作成せずに最終Edge Nodeを起動した。
read-only cutover preflightが既存の`edge_node_id`を受理し、BravePI MainboardのUARTから受信した
温度3観測が`pub_seq` 12〜14としてIoTKit Edgeへ保存された。Edge Nodeの`accepted-through` cursorは11から14へ進み、
IoTKit Edge queryで同じ`edge_node_id`・`ledger_epoch`と`temperature_c`の3 recordを確認した。停止後のEdge Node DBと
IoTKit Edge DBはともに`PRAGMA quick_check = ok`であり、Edge Node processの停止とUART解放も確認した。

### Host耐障害検証状況 (2026-07-14)

commit `0696d22`を使い、通常のEdge Node、Mosquitto、IoTKit Edgeだけでcustody失敗系とprocess境界の耐障害性を
検証した。テスト専用の製品failpointは追加していない。

- IoTKit Edgeの実SQLite transactionをtriggerで失敗させた場合、raw record、cursor、ackはいずれも0件のまま
  だった。同じbatchの再送は1行のまま冪等で、同じidentityを異なる内容で再送した場合はconflictとなり、
  元recordのbytesとcursorを保持して2つ目のackを返さなかった。
- Brokerを停止した状態でEdge Node outboxへ300件を作成し、Edge Nodeを再起動してもcursorが0、outboxが300件の
  まま保たれることを確認した。BrokerとIoTKit Edgeの復旧後、256件上限により2 batch以上でcursorとIoTKit Edgeが
  300へ収束した。
...
- Brokerが利用可能なままIoTKit Edgeを停止し、`pub_seq` 301を追加してEdge Nodeを再起動してもcursorは300から
  進まなかった。IoTKit Edge復旧後は301へ収束した。その後、Edge Node再起動をまたぐ302、Broker再起動をまたぐ303、
  IoTKit Edge再起動と通常retryをまたぐ304を順に確認した。
- 最終状態はEdge Node cursor 304、IoTKit Edge raw record 304件、最小1、最大304、distinct 304であり、欠番と重複は
  なかった。停止後のEdge Node DBとIoTKit Edge DBはいずれも`PRAGMA quick_check = ok`だった。
...
- 開始時はEdge Node停止、Broker/IoTKit Edge稼働、Edge Node readings 18件、current-epoch `pub_seq`最大18、Edge Node cursorと
  IoTKit Edge raw recordは17だった。Edge Node/IoTKit Edge DBはいずれも`PRAGMA quick_check = ok`だった。
- Broker/IoTKit Edgeを停止して最新Edge Nodeを起動すると、cursor 17のままreadingsは18から22へ増えた。さらに
  Edge NodeをSIGINTで停止して再起動し、修復操作なしでreadingsが26から32へ増えたため、downstream停止中の
  SQLite収集継続とEdge Node再起動後のUART再取得を確認した。
- Broker/IoTKit Edge復旧後、停止中のrecordを再送してEdge Node cursorとIoTKit Edgeが17から39以上へ追いついた。その間も
...
- 最終的にEdge Node readings 127件、current-epoch `pub_seq`最大127、validated `accepted-through` cursor 127、
  IoTKit Edge raw record 127件（最小1、最大127、distinct 127）へ収束した。Edge Node/IoTKit Edge DBはいずれも
  `PRAGMA quick_check = ok`であり、Edge NodeをSIGINTで停止した後にUARTが解放された。Broker/IoTKit Edge、lab、
...
- IoTKit Edgeをarchive-responsible target、cursor 0とした状態で、90 envelopeのreading、measurement outbox、
...
- 失敗後も3テーブルの件数は90/90/90で一致し、失敗したenvelopeの部分行やdedup claimは残らなかった。
  IoTKit Edge cursorは0のままで、DBは設定したpage上限を超えず、`PRAGMA quick_check = ok`だった。
- 上限を広げてCollectorを停止し、同じDBを再オープンしても90/90件のreading/outboxを保持していた。
  失敗した同一envelopeを再送すると受理され、reading、outbox、dedupは91/91/91へ回復した。
- 続けて通常のEdge Node、Mosquitto、IoTKit Edgeによる耐障害scriptを新規環境で再実行し、Edge Node/IoTKit Edge再起動、
  Broker再起動、IoTKit Edge単独停止をまたいで304 recordが欠番・重複なく再収束した。

これにより `BravePI Transmitter -> BLE Long Range -> BravePI Mainboard -> UART -> IoTKit Edge Node SQLite -> MQTT
-> MQTT Broker -> IoTKit Edge raw SQLite -> accepted-through -> Edge Node cursor` は実機確認済みとなった。
...
- YokaKit連携、UI、dashboard、IoTKit Edge projection、cloud/fleet管理
- 第三者デバイスingress、pairing、オンボーディングUI
- credential enrollment/rotation、multi-Edge Node運用、MQTT Broker HA
- IoTKit Edge backup/restore、archive repair、汎用fan-out
...
含めない。ゲート完了直後の実装スライスでは、IoTKit Edgeでcanonical sensor seriesへ当時の
...
### IoTKit Edge semantic sliceのhost live-broker検証 (2026-07-15)

Docker上のMosquitto 2.0.22とIoTKit Edgeを一時環境で起動し、疑似Edge Node、IoTKit Edge、application subscriberを
...
enqueueして配送状態を確認できるようにした。新規Edge Node DBを初期化し、通常のEdge Node、Mosquitto、IoTKit EdgeをDockerの
一時環境で起動した後、`iotkit-edge-nodectl smoke enqueue`が返したepoch/pub_seqを`smoke status`へ渡した。
レコードは通常outboxとEdge Node固有topicを通ってIoTKit Edge raw SQLiteへ耐久保存され、同じtest_idをIoTKit Edge queryで確認し、
...
raw recordをIoTKit Edge storeへ受理すると、semantic event 1行と`published_at IS NULL`のoutbox 1行が生成・保持された。
同じIoTKit Edge processを維持してbrokerを復旧すると、QoS 1でapplication subscriberへ同じ`event_id`が1回届き、
outboxのpendingは1件から0件へ収束した。mapping revision境界とroute作成前eventの非配送は今回再検証せず、
既存focused testの範囲に残す。

### 実運用形IoTKit Edge bootstrap検証 (2026-07-15)

Edge NodeをRaspberry Pi上のnative process、標準BrokerとIoTKit EdgeをLinux IoTKit Edge host上のDocker Composeとする導入経路を
追加した。`iotkit-edge-nodectl mqtt-binding`の非secret JSON、明示したBroker hostname/bind address、operatorが
用意したserver certificate/key/CAから、匿名禁止設定、Edge Node単位のexact topic ACL、hash済みBroker
password database、IoTKit Edge credential、Edge Nodeへ安全に引き渡すcredential/CA/TOML fragmentを生成する。
VPNや特定network製品、certificate発行はこのbootstrapの要件・責務に含めない。

一時的な新規Edge Node DBと自己署名test CAを使い、生成した構成で実際のMQTT TLS接続、IoTKit Edge raw耐久保存、
...
- IoTKitの責任はUART streamの受信から始まり、frame decode、正規化、耐久保存、IoTKit Edgeへの引き渡しを担う。
...
入力を受ける。IoTKitもYokaKitをcore、IoTKit Edgeの汎用semantic model、adapter境界へ組み込まず、
...
自社現場(Wave 0稼働中のEdge Node)に対して行う。
...
未所有中はnetwork API/UIをbindせず、local `iotkit-edge-nodectl`で所有権確立後にoperatorトークン認証へ入る。
