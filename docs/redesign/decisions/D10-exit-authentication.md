...
最初のGateway-to-Site実機スライスは、ユーザー管理のTailscale tailnet内で標準MQTT brokerへ接続する。
overlayは到達経路とdefense in depthであり、MQTT application認証の代替ではない。

MVPの認証は次に限定する。

- brokerは匿名接続を禁止する。
- Gatewayごとにstatic credentialを1つ発行する。共有credentialを使わない。
- usernameはGateway identityへ束縛し、ACLは当該Gatewayのrecords publishとaccepted-through subscribeだけを許可する。
...
最初の1 Gateway構成では、credentialの作成・配布・失効はoperatorが行う。自動enrollmentや自動rotationを
実装する前に、実機の収集・配送・再送・照会を完成させる。

## Authority

MQTT endpointを運営するSite operatorがbroker credentialとACLの発行権威である。Gatewayは受け取った
credentialをtarget設定へ束縛し、別Gateway namespaceへ使わない。

Tailscale account、node admission、ACL、account recoveryはoverlay providerとsite operatorの責務であり、
IoTKitはprovider secretを保存しない。Tailscale nodeであることだけをGateway認証として扱わない。

## Failure behavior

- 認証失敗やACL違反ではpublicationをSite custodyとして扱わない。
- credential失効後はbroker接続を拒否または切断する。
- brokerまたはoverlay停止中もGatewayはoutboxを保持する。
- credential喪失時はoperatorが新しいcredentialを発行し、Gateway設定を更新する。データcursorはcredentialと独立して維持する。
...
複数Gatewayを第三者へ配布する段階で、実際の設置journeyと脅威モデルを基に別途判断する。それまでは
この一覧を実装計画へ展開しない。
