...
Status: 確定、2026-07-18 Edge Node activation ACL追記

## 決定

Edge Nodeは、配置環境が用意する到達可能なIP networkを通じて標準MQTT brokerへ接続する。経路は同一拠点LAN、
...
- brokerは匿名接続を禁止する。
- Edge Nodeごとにstatic credentialを1つ発行する。共有credentialを使わない。IoTKit EdgeにもEdge Nodeとは別の
  IoTKit Edge固有credentialを発行し、全主体でcredentialを共有しない。
- usernameは`edge_node_id`へ束縛し、ACLは当該Edge Nodeのrecords/descriptors/activation result publishと
  accepted-through/activation request subscribeだけを許可する。IoTKit Edgeは全Edge Nodeの
...
BrokerとIoTKit Edgeの同一host配置はreference deploymentの一つであり、契約上の必須要件ではない。Broker、IoTKit Edge、
Edge Nodeは別hostに配置でき、共有filesystemや同一Docker Compose projectを接続条件にしない。Mosquittoの
certificate/秘密鍵、listener、password database、ACLはBroker hostへ、IoTKit Edge固有のclient credentialはIoTKit Edge
hostへ、Edge Node固有のclient credentialは各Edge Node hostへ置く。
...
- MQTT client ID
- `edge_node_publish`、`edge_ingest`、`edge_external_publish`等のprincipal role
...
endpointの登録・変更・profile切替をIoTKit Consoleから行わない。

Edge Nodeは初回自己構成で`edge_node_id`を生成した後、Broker operatorがそのIDに束縛したcredential/ACLと
handoff bundleを生成する。初版はIoTKit ConsoleからEdge Nodeへ遠隔配布せず、各Edge Nodeのlocal CLIでhandoffを
適用する。IoTKit EdgeはIoTKit Edge固有profileを別に受け取り、Brokerと同居していてもnetwork clientとして認証する。
IoTKit Edgeから外部Brokerへpublishする場合も、外部Broker用の別profileをIoTKit Edge hostへinstallする。

このBroker enrollmentは通信許可であり、IoTKit Edge raw historyへの参加許可ではない。Edge Node activationは、既に
認証されたEdge Nodeのexact ledger incarnationへ将来のrecords受理を許可する独立したapplication操作である。
Consoleはactivation IDとIoTKit Edge DB上のgrantを作るが、Broker credentialやACLを変更しない。

接続testはDNS、TCP、TLS chain/server name、MQTT authentication、CONNACKまでを確認する。Edge NodeからIoTKit Edgeへの
最終commissioningは、実recordがIoTKit Edgeへ耐久保存され`accepted-through`がEdge Nodeへ返るまで確認する。IoTKit Edgeから
...
Linux production hostではOS非依存の一回実行commandをsystemd timerから起動する。Windows IoTKit Edge/Broker hostの
...
別hostのIoTKit Consoleは、実TLS接続で観測したpeer certificateの期限、最終観測時刻、接続・配送状態だけを
非秘密のread modelとして表示し、Broker内部の更新成功を推測しない。初版は更新status専用の遠隔管理経路を
追加しない。Consoleはcertificate発行、upload、接続設定変更、profile切替を行わない。

## Authority

MQTT endpointを運営するBroker operatorがbroker credentialとACLの発行権威である。同一人物がIoTKit Edge operatorを
兼ねてもよいが、同一hostや同一管理componentを要件にしない。IoTKit Edge Nodeは受け取った
credentialをtarget設定へ束縛し、別Edge Node namespaceへ使わない。

LAN、拠点間回線、VPN、名前解決、firewall等の到達経路は配置環境とdeployment operatorの責務である。VPNを使う
...
- 認証失敗やACL違反ではpublicationをIoTKit Edge custodyとして扱わない。
- 認証に成功しても、Edge Node activationが未完了またはledger epochが不一致ならrecordsを保存せずackしない。
- activation request/resultのPUBACK、retained descriptor、通常再接続はactivation完了やデータ削除を意味しない。
- credential失効後はbroker接続を拒否または切断する。
- brokerまたはnetwork経路の停止中もIoTKit Edge Nodeはoutboxを保持する。
- credential喪失時はoperatorが新しいcredentialを発行し、Edge Node設定を更新する。データcursorはcredentialと独立して維持する。
...
- anonymous、誤password、別Edge Node namespace、過大権限、誤CA、誤hostname、期限切れcertificate、平文listenerを
...
`scripts/test-mqtt-security.sh`は、匿名、誤password、別Edge Node namespace、IoTKit Edgeの過大権限、誤CA、誤hostname、
...
- IoTKit Edge backup/restore時のcredential reconciliation
- fleet enrollment、canary rotation、decommission automation
- deactivation/reactivation、IoTKit Edge transfer、Edge Node ID reuse
...
- 第三者へ多数のEdge Nodeを配布し、個別passwordの安全な配布・失効を運用で保証できない。
- 制御、安全性に関わるtopic、またはclone耐性を必要とする。
- 顧客、規制、接続先外部Brokerがclient certificateを要求する。

通常fileに置いたmTLS private keyは盗難・複製されたstatic passwordと同様にcloneでき、侵害済みEdge Node、Broker
