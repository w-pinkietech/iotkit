...
- **per-deviceトークン**(共有キー禁止)。IoTKit Edgeはハッシュ保存、失効はデバイス単位、
...
- **TLSデフォルト**: Edge自己署名証明書+オンボーディング時フィンガープリント配布(ピン留め)。
...
- **dry_run/validateエンドポイント**(認証込み・副作用なしの試行)+Edge UIのライブインスペクタ
...
- **time_qualityはエンベロープに載せない(2026-07-03、同上)**: R18の時刻品質(synced/holdover/unsynced)は
  **受信側(IoTKit Edge)が自分の時計状態を評価して刻む**受信時メタデータであり、送信者は主張できない。
