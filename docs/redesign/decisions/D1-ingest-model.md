...
- `time_source` タグ: device_ntp / device_rtc / edge_node / edge_node_adjusted。
...
- **per-deviceトークン**(共有キー禁止)。IoTKit Edge Nodeはハッシュ保存、失効はデバイス単位、
...
- **TLSデフォルト**: Edge Node自己署名証明書+オンボーディング時フィンガープリント配布(ピン留め)。
...
- **物理アクション前の検証ゲート**: 接点出力等は「検証済み・値域内・鮮度OK」データのみ+N連続/持続時間条件。
  ※接点出力の駆動自体はD12決定1(2026-07-08)で今回の設計から繰り延べ——本ゲートは将来の有効化条件として維持。
...
- **dry_run/validateエンドポイント**(認証込み・副作用なしの試行)+Edge Node UIのライブインスペクタ
...
- **AIオペレーター経由のプロンプトインジェクション対策**(セキュリティ節に追加):
  - operatorトークン(AI用)は物理アクション権限段階に**昇格不可**。物理アクションを伴う操作は人間承認必須。
    ※「物理アクション」=現場状態を変える出力(D12決定1の線引き規則)。デバイス再起動等の機器の世話は
    該当しない(世話の権限はD12決定3の3分類に従う)。
...
- **time_qualityはエンベロープに載せない(2026-07-03、同上)**: R18の時刻品質(synced/holdover/unsynced)は
  **受信側(IoTKit Edge Node)が自分の時計状態を評価して刻む**受信時メタデータであり、送信者は主張できない。
