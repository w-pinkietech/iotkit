...
MQTT PUBACKとapplication custody ackは異なる。

- **PUBACK**: brokerがMQTT publicationを受領したことだけを表す。
- **accepted-through**: Site Serverがraw recordと連続cursorを耐久保存したことを表す。
...
- 同じrecord identityで異なる内容を受信した場合はcustody conflictとし、cursorを進めない。
...
brokerの永続queueは追加の配送bufferとして使ってよいが、Gatewayのcustody責任を代替しない。
...
正式水位がapplication-level accepted-throughであるため、PUBACKをcustodyと同一化する必要はない。
