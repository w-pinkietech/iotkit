...
| `trial-sample-adapter` | `edge-node/adapters/trial-sample` | Explicitly configured local-only sample Input Adapter for the trial profile; emits changing measurements through the standard adapter host and custody path and is never enabled by default. |
