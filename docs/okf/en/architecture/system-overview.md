...
| `trial-sample-adapter` | `edge-node/adapters/trial-sample` | Explicitly configured local-only sample Input Adapter for the trial profile; emits changing measurements through the standard adapter host and custody path. Field enablement requires `IOTKIT_ENABLE_TRIAL_SAMPLE=1`, and its inventory model id is non-hardware (`trial-sample-illuminance`). |
