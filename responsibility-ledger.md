...
| R8 | 受理の権威(コレクタ) | 重複排除・確認応答・逆圧(ストレージ水位と連動) | 旧(d)維持 |
...
- R3(正規化)の実行はアダプタ内で3分掌: デコード=ドライバ / レジストリ・series写像=アダプタランタイム / 較正=R9([terminology.md](terminology.md) の「正規化」参照)
...
- 論点2(次): コマンドパイプラインの部品分割、コンポーネント境界の確定(iotkit-nextクレート再編)
